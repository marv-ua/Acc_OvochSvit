...
// Test}		
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Выборка.ФизическоеЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
			Если ЗначениеЗаполнено(Выборка.НомерПриказа) И ЗначениеЗаполнено(Выборка.ДатаНачала) Тогда
				Строка["T5T1RXXXXG16S"] = "Договір підряду" + " № " + РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Выборка.НомерПриказа, Истина, Истина) + " від " + Формат(Выборка.ДатаНачала, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
			КонецЕсли;
		КонецЕсли;
		// Test}
