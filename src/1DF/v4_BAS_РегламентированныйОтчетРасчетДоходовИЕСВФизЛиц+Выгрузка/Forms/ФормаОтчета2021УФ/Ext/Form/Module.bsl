...
// Test}		
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			Если ЗначениеЗаполнено(Выборка.НомерПриказа) И ЗначениеЗаполнено(Выборка.ДатаПриказа) Тогда
				Строка["T5T1RXXXXG16S"] = "Наказ № " + РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Выборка.НомерПриказа, Истина, Истина) + " від " + Формат(Выборка.ДатаПриказа, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
			КонецЕсли;			
		КонецЕсли;
		// Test}
       ...
       Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		Для Каждого стр ИЗ ТаблицаНДФЛ Цикл
			...
			Если ТипЗнч(стр.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
				Строка[ПрефиксИмениКолонки+"GFIO"] = стр.Контрагент.НаименованиеПолное;
			КонецЕсли;
			// Test}
			...
			// Test{20230929
			Если Не стр.ДоходНДФЛ = "157" Тогда
				Строка[ПрефиксИмениКолонки+"G03"] = стр.ДоходПриход; 
				Строка[ПрефиксИмениКолонки+"G04"] = стр.НалогПриход;
				Строка[ПрефиксИмениКолонки+"G5"] = стр.ВСПриход;
			КонецЕсли;
			Строка[ПрефиксИмениКолонки+"G5"] = стр.ВСПриход;	
			// Test}
...
//	НовСтрока.НазваниеСтраницыПанели ="П1T1R";
	...
	НовСтрока = тбСхемыXML.Добавить();
	НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0510409", "F0510409"); 
