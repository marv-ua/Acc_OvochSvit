...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2023Кв4УФ";
...
Процедура ЗаполнитьПриложениеРИ_ТЦ(ДатаНачала, ДатаКонца, НомерПер)
	...
	Если ИДКонф = "ЕРП" И Метаданные.РегистрыСведений.Найти("СвязанностьЛицКонтролируемыхОпераций") <> Неопределено и СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Год Тогда
...
Процедура ЗаполнитьУбытокПрошлогоГода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если ГодПериода = 2022 Тогда 
			//две схемы "22" и "23" только для года 2022 
		...
		ИначеЕсли ГодПериода = 2023 Тогда  //2023
...
Процедура ЗаполнитьПоказателиПрошогоПериода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если Месяц(ДатаПрошлогоКвартала) = 9 и ГодПериода = 2023 Тогда 
			Если ЮрЛицо Тогда  
		...
		Если ПараметрыЗаполненияЗвит1С.Свойство("ПараметрыЗаполненияПоказателиПрошогоПериода", ПараметрыЗаполненияПоказателиПрошогоПериода) Тогда
			...
			Попытка
				...
				Иначе
					...
					Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета>= '20230930' и (Не Найти(глМенеджерЗвит1С.ИмяСхемыЗвит1С,"01005")> 0) ТОгда 
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если ЮрЛицо Тогда
		...
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0100525", "J0100125");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151825", "J0111825");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150325", "J0110325");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152825", "J0112825");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150625", "J0110625");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151325", "J0111325");  
		...
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151425", "J0111425");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела1.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151525", "J0111525");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151625", "J0111625");  
		...
		Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "Декларация")) Тогда
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151725", "J0111725");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152025", "J0112025");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела2.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154425", "J0114425");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154825", "J0114825");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154925", "J0114925");  
