...
//		|	КОНЕЦ 
	...
	//Таблица дат начала и окончания периода трудовых отношений для категорий сотрудников
	...
	Если ЗарплатаКадрыРасширенная Тогда
		...
		|   СотрудникиОрганизации.ДатаНачала КАК ДатаНачала,
		|   СотрудникиОрганизации.ДатаОкончания КАК ДатаОкончания,
		|   СотрудникиОрганизации.ПризнакГПХ КАК ПризнакГПХ
		...
		|ИЗ
		|ВТСотрудникиОрганизации КАК СотрудникиОрганизации
		|ГДЕ СотрудникиОрганизации.Сотрудник Не В (ВЫБРАТЬ ВТДоговораГПХ.Сотрудник ИЗ ВТДоговораГПХ КАК ВТДоговораГПХ)
		|СГРУППИРОВАТЬ ПО
		|   ФизическоеЛицо
		|
		|ОБЪЕДИНИТЬ ВСЕ
	    ...
	    |СГРУППИРОВАТЬ ПО
		|   ДоговораГПХ.Сотрудник.ФизическоеЛицо
		|) КАК СотрудникиОрганизации
		|ИНДЕКСИРОВАТЬ ПО
		|	ФизическоеЛицо";
		Запрос.Выполнить();
	...
	Если ЗарплатаКадрыРасширенная Тогда
		...
		|	ВЫБОР
		|		КОГДА КадроваяИсторияСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы) ИЛИ КадроваяИсторияСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)		
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК Категория,
		|	ВЫБОР
		...
		|	ВЫБОР
		|		КОГДА СотрудникиОрганизации.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы) ИЛИ СотрудникиОрганизации.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК Категория,
		|	ВЫБОР
	...
	|	ПринятыеУволенныеВТекущемПериоде.ВидЗанятости КАК ВидЗанятости,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтруктураРеквизитовФормы.ОпцияКадровыйПереводДвумяСтроками И Выборка.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение И Выборка.Категория <> 3 И (НачалоДня(Выборка.ДатаНачала)-1)>= СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета Тогда
			...
			Строка["T5T1RXXXXG14S"]  	= СокрЛП(Выборка.ДолжностьНаименованиеПоКППредыдущий);
			Если Выборка.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение Тогда
				Строка["T5T1RXXXXG13"]  = 1;
			КонецЕсли;
			Строка["T5T1RXXXXG12"]		= Выборка.ВидЗанятости;
			Строка["T5T1RXXXXG16S"]  	= СокрЛП(Выборка.ДолжностьКодКППредыдущий);
			Строка["T5T1RXXXXG15S"]  	= СокрЛП(Выборка.ДолжностьПредыдущий);
			Строка["T5T1RXXXXG17S"]  	= "№ " + РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Выборка.ОснованиеНомер, Истина, Истина) + " від " + Формат(Выборка.ОснованиеДата, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
		...
		Если Выборка.Категория <> 3 Тогда
			Строка["T5T1RXXXXG14S"]  	= СокрЛП(Выборка.ДолжностьНаименованиеПоКП);
			Если Выборка.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение Тогда
				Строка["T5T1RXXXXG13"]  = 1;
			КонецЕсли;
			Строка["T5T1RXXXXG12"]  	= Выборка.ВидЗанятости;
			Строка["T5T1RXXXXG16S"]  	= СокрЛП(Выборка.ДолжностьКодКП);
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.ОснованиеНомер) И ЗначениеЗаполнено(Выборка.ОснованиеДата) Тогда
			Строка["T5T1RXXXXG17S"]  	= "№ " + РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Выборка.ОснованиеНомер, Истина, Истина) + " від " + Формат(Выборка.ОснованиеДата, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
		КонецЕсли;	
		Строка["T5T1RXXXXG18S"]  	= Выборка.СтатьяЗакона;
		
		Если ЗначениеЗаполнено(Выборка.ДатаСозданияРабочегоМеста) Тогда
			Строка["T5T1RXXXXG19D"]  = Выборка.ДатаСозданияРабочегоМеста;;
		КонецЕсли;		
	...
	Если ЗарплатаКадрыРасширенная Тогда
		...
		|	ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОтпускБезОплаты) КАК КатегорияНачисленияИлиНеоплаченногоВремени,
	...
	|       И ВЫБОР КОГДА ЕСВФОТ.ВидЕСВ <> ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоДоговорамГПХ)
	|       	ТОГДА ПериодыТО.ПризнакГПХ = ЛОЖЬ
	|           ИНАЧЕ ПериодыТО.ПризнакГПХ = ИСТИНА
	|       КОНЕЦ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДоначислениеФОТ <> 0 ИЛИ ДоначислениеНалогФОТ <> 0 Тогда
			Если Строка["T6T1RXXXXG17"] = 0 И Строка["T6T1RXXXXG18"] = 0 И Строка["T6T1RXXXXG20"] = 0 
				И Выборка.ОтработанныеДни = 0 И Выборка.КалендарныеДниОтпускаБезСохраненияЗП = 0 Тогда
