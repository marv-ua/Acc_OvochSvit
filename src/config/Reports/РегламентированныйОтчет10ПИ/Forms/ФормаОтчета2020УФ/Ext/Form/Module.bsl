...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Иначе	
		...
		СельскиеЖители = Новый Массив;
		мСельскиеЖители = КадровыйУчетБазовый.ПолучитьСписокСельскихЖителей(СтруктураРеквизитовФормы.Организация, СельскиеЖители);
		
		// Численность проживающих в сельских населенных пунктах и ПГТ
		ПараметрыСредняяЧисленность = Новый Структура;
		ПараметрыСредняяЧисленность.Вставить("ВидПериода", "КВАРТАЛСНАЧАЛАГОДА") ;
		ПараметрыСредняяЧисленность.Вставить("НачалоПериода", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		ПараметрыСредняяЧисленность.Вставить("КонецПериода", КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		...
		ПараметрыСредняяЧисленность.Вставить("ОтборИностранцы", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантии", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборСотрудники", мСельскиеЖители);
		ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		...
		|	И Сотрудники.ФизическоеЛицо.ДатаРождения <> ДАТАВРЕМЯ(1,1,1)
		|	И Сотрудники.ФизическоеЛицо.ДатаРождения МЕЖДУ &ОкончаниеИнтервала И &НачалоИнтервала";
		Запрос.УстановитьПараметр("Организация", СтруктураРеквизитовФормы.Организация);
		Запрос.УстановитьПараметр("НачалоИнтервала", КонецМесяца(ДобавитьМесяц(КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), -18*12)));
		Запрос.УстановитьПараметр("ОкончаниеИнтервала", НачалоГода(ДобавитьМесяц(КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), -35*12)));
		СотрудникиОт18До35 = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Сотрудник");
		...
		ПараметрыСредняяЧисленность.Вставить("ОтборИностранцы", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантии", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборСотрудники", СотрудникиОт18До35);
		ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		...
		|	И Сотрудники.ФизическоеЛицо.ДатаРождения <> ДАТАВРЕМЯ(1,1,1)
		|	И Сотрудники.ФизическоеЛицо.ДатаРождения МЕЖДУ &ОкончаниеИнтервала И &НачалоИнтервала";
		Запрос.УстановитьПараметр("НачалоИнтервала", КонецМесяца(ДобавитьМесяц(КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), -36*12)));
		Запрос.УстановитьПараметр("ОкончаниеИнтервала", НачалоГода(ДобавитьМесяц(КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), -60*12)));
		СотрудникиОт36До60 = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Сотрудник");
		...
		ПараметрыСредняяЧисленность.Вставить("ОтборИностранцы", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантии", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборСотрудники", СотрудникиОт36До60);
		ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		...
		|	И Сотрудники.ГоловнаяОрганизация = &Организация
		|	И Сотрудники.ФизическоеЛицо.ДатаРождения <> ДАТАВРЕМЯ(1,1,1)
		|	И Сотрудники.ФизическоеЛицо.ДатаРождения < &НачалоИнтервала";
		Запрос.УстановитьПараметр("НачалоИнтервала", НачалоГода(ДобавитьМесяц(КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), -60*12)));
		СотрудникиОт60 = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Сотрудник");
		...
		ПараметрыСредняяЧисленность.Вставить("ОтборИностранцы", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантии", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборСотрудники", СотрудникиОт60);
		ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		...
		ПараметрыПолученияСотрудников.Организация = СтруктураРеквизитовФормы.Организация;
		ПараметрыПолученияСотрудников.НачалоПериода = НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		ПараметрыПолученияСотрудников.ОкончаниеПериода = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета;
		ПараметрыПолученияСотрудников.КадровыеДанные   = "ВидЗанятости";
	
		ТаблицаСотрудниковОрганизации = КадровыйУчет.СотрудникиОрганизации(Истина, ПараметрыПолученияСотрудников);
		
		ЗапросОбшийФОТ.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		ЗапросОбшийФОТ.УстановитьПараметр("ТаблицаСотрудниковОрганизации", ТаблицаСотрудниковОрганизации);
		
		ЗапросОбшийФОТ.Текст = "
		|ВЫБРАТЬ  
		|  ТаблицаСотрудниковОрганизации.ФизическоеЛицо КАК ФизическоеЛицо
		|ПОМЕСТИТЬ ВТТаблицаСотрудниковОрганизации 
		|ИЗ &ТаблицаСотрудниковОрганизации КАК ТаблицаСотрудниковОрганизации
		|ГДЕ 
		|  ТаблицаСотрудниковОрганизации.ВидЗанятости <> Значение(Перечисление.ВидыЗанятости.Совместительство)
		|ИНДЕКСИРОВАТЬ ПО
		|	ФизическоеЛицо
		|";
		ЗапросОбшийФОТ.Выполнить();
		...
		|                                                                                     ВТТаблицаСотрудниковОрганизации.ФизическоеЛицо КАК ФизическоеЛицо
		|                                                                                     ИЗ ВТТаблицаСотрудниковОрганизации КАК ВТТаблицаСотрудниковОрганизации)
