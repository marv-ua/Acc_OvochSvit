...
Процедура ВывестиСотрудникаРасчетныйЛисток(СтрокаОсновноеМестоРаботы, ДокументРезультатПромежуточный, Макеты, СоответствиеПользовательскихПолей, ДанныеПоНДФЛ)
	...
	КонецЕсли;
	СтруктураОтбораПоВидуЗанятости = Новый Структура("ВидЗанятости", Перечисления.ВидыЗанятости.ВнутреннееСовместительство);
	МассивСотрудникаВнутреннееСовместительство = СтрокаОсновноеМестоРаботы.Строки.НайтиСтроки(СтруктураОтбораПоВидуЗанятости);
	Для Каждого СтрокаСотрудникаВнутреннееСовместительство Из МассивСотрудникаВнутреннееСовместительство Цикл  
