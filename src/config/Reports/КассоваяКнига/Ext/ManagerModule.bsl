...
Процедура СформироватьЛистыКассовойКниги(ПараметрыОтчета)
	...
	Пока РезультатПоРегистратору.Следующий() Цикл
		Если РезультатПоРегистратору.Период <> ТекДата Тогда
			...
			ОтветственныеЛица = ПолучитьОтветственныеЛицаКассовойКниги(ПараметрыОтчета, КонецДня(ТекДата));
...
Функция ПолучитьОтветственныеЛицаКассовойКниги(Параметры, ДатаСреза)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.ОбособленноеПодразделениеОрганизации) Тогда
		Руководители = ОтветственныеЛицаБП.ОтветственныеЛица(Параметры.Организация, ДатаСреза);
	Иначе
		Руководители = ОтветственныеЛицаБП.ОтветственныеЛицаОбособленногоПодразделения(Параметры.ОбособленноеПодразделениеОрганизации, ДатаСреза);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Руководители.ГлавныйБухгалтерФИО) И Не ЗначениеЗаполнено(Результат.ГлавныйБухгалтерПредставление) Тогда
		Результат.ГлавныйБухгалтерПредставление   = ОбщегоНазначенияБПВызовСервера.ПолучитьФамилиюИмяОтчество(Руководители.ГлавныйБухгалтерФИО.Фамилия, Руководители.ГлавныйБухгалтерФИО.Имя, Руководители.ГлавныйБухгалтерФИО.Отчество, Истина); // Кратко
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Руководители.КассирФИО) И НЕ ЗначениеЗаполнено(Результат.КассирПредставление) Тогда
		Результат.КассирПредставление      = ОбщегоНазначенияБПВызовСервера.ПолучитьФамилиюИмяОтчество(Руководители.КассирФИО.Фамилия, Руководители.КассирФИО.Имя, Руководители.КассирФИО.Отчество, Истина); // Кратко
	КонецЕсли;
