...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ПараметрыПолученияСотрудников.НачалоПериода = ДатаНач;
	ПараметрыПолученияСотрудников.ОкончаниеПериода = ДатаКон;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Строка["T1RXXXXG7S"] = РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Результат.НомерПриказа, Истина, Истина);
