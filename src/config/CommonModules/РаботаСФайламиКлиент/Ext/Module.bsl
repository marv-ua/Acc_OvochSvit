...
Асинх Процедура СохранитьКак(ТабличныйДокумент, ПредлагаемоеИмя, Знач Фильтр = "") Экспорт
	
	Если Фильтр = "" Тогда
		Фильтр = "Excel (*.xlsx)|*.xlsx";
	КонецЕсли;
	
	СтруктураФайла 	= Ждать ВыбратьФайл( , ПредлагаемоеИмя, Фильтр, Истина);
	Обещание 		= Ждать СохранитьРезультатНаТонкомКлиенте(ТабличныйДокумент, СтруктураФайла);
		
КонецПроцедуры

Процедура ПаузаСС(КвоСекунд) Экспорт
...
Асинх Функция СохранитьРезультатНаТонкомКлиенте(ТабличныйДокумент, СтруктураФайла) Экспорт 
	
	Если СтруктураФайла = Неопределено или СтруктураФайла.ПолноеИмя = "" Тогда
		Сообщить("Не выбран файл!");
		Возврат Неопределено;
	КонецЕсли;
	
	Обещание = Ждать ТабличныйДокумент.ЗаписатьАсинх(СтруктураФайла.ПолноеИмя, ТипФайлаТабличногоДокумента.XLSX);
		
	Возврат Обещание;
КонецФункции

&НаКлиенте
Асинх Функция ВыбратьКаталог() Экспорт
...
&НаКлиенте
Асинх Функция ВыбратьФайл(Каталог = "", ПолноеИмя = "", Фильтр = "", Сохранение = Ложь) Экспорт
	
	СтруктураФайла 						= Новый Структура("Имя, ПолноеИмя, ИмяБезРасширения, Расширение, Путь, ВыбранныйФайл");
	
	ДиалогОткрытия 						= Новый ДиалогВыбораФайла(?(Сохранение, РежимДиалогаВыбораФайла.Сохранение, РежимДиалогаВыбораФайла.Открытие));
	
    ДиалогОткрытия.Фильтр 				= Фильтр; //"Внешние обработки (*.epf)|*.epf|Внешние отчеты (*.erf)|*.erf";
    ДиалогОткрытия.МножественныйВыбор 	= Ложь;
    ДиалогОткрытия.Заголовок 			= ?(Сохранение, НСтр("ru='Сохранить как...';uk='Зберегти як...'"), НСтр("ru='Открыть файл...';uk='Відкрити файл...'"));
	ДиалогОткрытия.Каталог 				= Каталог;
	ДиалогОткрытия.ПолноеИмяФайла 		= СтрЗаменить(ПолноеИмя, Каталог, "");

    Обещание 							= Ждать ДиалогОткрытия.ВыбратьАсинх();
	
	Если ТипЗнч(Обещание) <> Тип("Массив") или Обещание.Количество() = 0 Тогда
		Возврат СтруктураФайла;
	КонецЕсли;
	
	ВыбранныйФайл 		= Новый Файл(Обещание[0]);
	
	ЗаполнитьЗначенияСвойств(СтруктураФайла, ВыбранныйФайл);
...
&НаКлиенте
Асинх Функция ПоместитьФайлВоВременноеХранилище(ПолноеИмяФайла, МожноПоместитьКопию = Ложь) Экспорт
	
	Попытка
		Обещание 				= Ждать ПоместитьФайлНаСерверАсинх(,,, ПолноеИмяФайла, Новый УникальныйИдентификатор);
	Исключение
		
		Если МожноПоместитьКопию Тогда
			
			Файл 		= Новый Файл(ПолноеИмяФайла);
			Обещание 	= Ждать Файл.СуществуетАсинх();
			
			Если Обещание = Истина Тогда
				ФайлПолучатель 	= ПолучитьИмяВременногоФайла();
				Обещание 		= Ждать КопироватьФайлАсинх(ПолноеИмяФайла, ФайлПолучатель);
				Если ЗначениеЗаполнено(Обещание) Тогда
					Попытка
						Обещание 				= Ждать ПоместитьФайлНаСерверАсинх(,,, ФайлПолучатель, Новый УникальныйИдентификатор); 
					Исключение
						Сообщить("Файл не может быть обработан: " + ОписаниеОшибки() + "!", СтатусСообщения.ОченьВажное);
						Возврат "";
					КонецПопытки;
				КонецЕсли;
				//УдалитьФайлыАсинх(ФайлПолучатель);
			КонецЕсли;
		Иначе
			Сообщить("Файл не может быть обработан: " + ОписаниеОшибки() + "!", СтатусСообщения.ОченьВажное);
			Возврат "";
	...
	Если Обещание = Неопределено Тогда
		Сообщить("Ошибка помещения файла!", СтатусСообщения.ОченьВажное);
		Возврат "";
	ИначеЕсли Обещание.ПомещениеФайлаОтменено Тогда
		Сообщить("Отменено помещение файла!", СтатусСообщения.ОченьВажное);
		Возврат "";
	КонецЕсли;
	
	АдресФайлаОбработки 	= Обещание.Адрес;
	
	Возврат АдресФайлаОбработки;

КонецФункции

Асинх Функция СохранитьФайлИзВременногоХранилища(АдресВоВременномХранилище, Фильтр = "", Каталог = "", ПолноеИмя = "") Экспорт
	
	Если АдресВоВременномХранилище = Неопределено Тогда
		Сообщить("ВНИМАНИЕ! Нет файла для сохранения !", СтатусСообщения.ОченьВажное);
		Возврат Неопределено;
	КонецЕсли;
	
	СтруктураФайла 	= Ждать ВыбратьФайл(Каталог, ПолноеИмя, Фильтр, Истина);
	
	Если СтруктураФайла = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Обещание 		= Ждать ПолучитьФайлССервераАсинх( АдресВоВременномХранилище, СтруктураФайла.ПолноеИмя);
