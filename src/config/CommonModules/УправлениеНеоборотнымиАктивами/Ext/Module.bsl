...
Функция РасчетАмортизацииНМАРегл(ДатаРасчета, Организация, НематериальныйАктив = Неопределено, ОтражатьВБУ = Истина, ОтражатьВНУ = Истина, 
	...
	Пока ВыборкаПоНМА.Следующий() Цикл
		...
		Если НачислятьАмортизациюНУ И (ВыборкаПоНМА.АмортизацияОборотНУ > 0) Тогда
			
			Если ВыдаватьСообщения Тогда
				
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нал. расчет амортизации: для нематериального актива %1 амортизация в этом месяце уже начислялась';uk='Под. розрахунок амортизації: для нематеріального активу %1 амортизація в цьому місяці вже нараховувалася'"), ВыборкаПоНМА.НематериальныйАктив);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения, , , , );
			...
			Иначе
				...
				Иначе
					...
					Если СтрокаТаблицаАмортизацииБух.Метод_50_50 Тогда
						...
						Строка.АмортизацияЗаМесяцНУ = ?(ОтражатьВНалоговомУчете, СтрокаТЗ.СтоимостьНУ - СтрокаТЗ.АмортизацияНУ, 0);
						
					Иначе
						
						Строка.АмортизацияЗаМесяцБУ = СтрокаТаблицаАмортизацииБух.Амортизация;
						Строка.АмортизацияЗаМесяцНУ = ?(ОтражатьВНалоговомУчете, СтрокаТаблицаАмортизацииБух.АмортизацияНУ, 0);
