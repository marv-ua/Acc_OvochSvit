...
Функция ПолучитьКонтактыКонтрагента(Контрагент)
	...
	Иначе
		...
		ИначеЕсли СтрДлина(Контрагент.Identifier) = 8 Тогда
			Если Не Контрагент.IsFop = Неопределено И Контрагент.IsFop Тогда
			//ПервыйСимвол = Лев(Контрагент.Identifier, 1);
			//ВторойСимвол = Сред(Контрагент.Identifier, 2, 1);
			//Если Найти("1234567890", ПервыйСимвол) = 0 
			//		И Найти("1234567890", ВторойСимвол) = 0 Тогда
				Сп.Добавить(СокрЛП(Контрагент.Name));		
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
