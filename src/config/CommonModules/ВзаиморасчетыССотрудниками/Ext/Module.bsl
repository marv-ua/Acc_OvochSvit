	...
	Для Каждого ТипВедомости Из Метаданные.РегистрыСведений.ОплатаВедомостейНаВыплатуЗарплаты.Измерения.Ведомость.Тип.Типы()  Цикл
		...
		Если Метаданные.НайтиПоТипу(ТипВедомости).ПолноеИмя() = "Документ.ВедомостьНаВыплатуЗарплаты" Тогда
			...
			Если ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме") Тогда
				...
				ТекстЗапросаПоТипуВедомости = СтрЗаменить(ТекстЗапросаПоТипуВедомости, "ВедомостьЗарплата.СчетУчета", "ВЫБОР КОГДА НЕ ВедомостьЗарплата.Ссылка.ФорматОбмена30 ТОГДА ВедомостьЗарплата.СчетУчета ИНАЧЕ ВедомостьЗарплата.ГруппаУчетаНачислений.СчетУчета КОНЕЦ");
			Иначе	
			...
			Если ИмяТЧ = "ЗарплатаПодробно" Тогда
				...
				ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
					ТекстЗапросаПоТипуВедомости = СтрЗаменить(ТекстЗапросаПоТипуВедомости, "//И", "И Зарплата.СпособВыплаты = ЗНАЧЕНИЕ(Перечисление.СпособыВыплатыЗарплаты.ЧерезБанк) И Зарплата.ВыплаченностьЗарплаты = ЗНАЧЕНИЕ(Перечисление.ВыплаченностьЗарплаты.Выплачено) И (Зарплата.Банк = &Банк ИЛИ Зарплата.Ссылка.ФорматОбмена30)")
				Иначе	
