...
Процедура РазобранныеСтрокиВДокумент() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Док.Дата < ДобавитьМесяц(ТекущаяДатаСеанса(), -1) Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ЭлТовары Из МассивТовары Цикл
			Если ЭлТовары = "Відмова від замовлення" Тогда
		...
		//	Если Кво = 0 Тогда 
			...
			Для Каждого строкаТовары Из Об.Товары Цикл
				Если строкаТовары.ДополнительныйАртикул = Артикул Тогда
					...
					Если НЕ ЗначениеЗаполнено(строкаТовары.СхемаРеализации) Тогда
						строкаТовары.СхемаРеализации = Справочники.СхемыРеализации.Товары;
					КонецЕсли;
					Если Не ЗначениеЗаполнено(строкаТовары.НалоговоеНазначение) Тогда 
						строкаТовары.НалоговоеНазначение = ПредопределенноеЗначение("Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая");
					КонецЕсли;
		...
		//	Иначе
			Если Не ТоварНайден Тогда 
				...
				Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовара.СхемаРеализации) Тогда
					НоваяСтрокаТовара.СхемаРеализации = Справочники.СхемыРеализации.Товары;
				КонецЕсли;
				Если Не ЗначениеЗаполнено(НоваяСтрокаТовара.НалоговоеНазначение) Тогда 
					НоваяСтрокаТовара.НалоговоеНазначение = ПредопределенноеЗначение("Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая");
				КонецЕсли;				
