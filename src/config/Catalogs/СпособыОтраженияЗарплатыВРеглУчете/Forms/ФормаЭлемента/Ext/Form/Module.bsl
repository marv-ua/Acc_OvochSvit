...
&НаКлиенте
Процедура УстановитьДоступностьКредита(Форма)
	
	Если ИспользуетсяУчетВоВнешнейПрограмме() Тогда
		Форма.Элементы.ГруппаКт.ТолькоПросмотр = Истина;
	Иначе
		Форма.Элементы.ГруппаКт.ТолькоПросмотр = Ложь;
	КонецЕсли;	
...
&НаСервере
Функция ИспользуетсяУчетВоВнешнейПрограмме()
	
	Возврат ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме");
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьДоступностьСпособОтраженияЕСВФОТ(ЭтаФорма);	
	УстановитьДоступностьКредита(ЭтаФорма);	
