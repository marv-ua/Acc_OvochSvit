...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Test{20230912
	КонтрагентыПриСозданииНаСервере();
	// Test}
...
//
//}}MRG[ <-> ]
	Если ЗначениеЗаполнено(ТекущийОбъект.КодПДФО) 
		И ТекущийОбъект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо 
		И Не ТекущийОбъект.КодПДФО = Справочники.ВидыДоходовНДФЛ.Код42
		И Не ЗначениеЗаполнено(ТекущийОбъект.КодВоенныеСбор)
		Тогда
		
		Сообщить("Не заполнен военный сбор");
		Отказ = Истина;
...
// Test{20230912
&НаСервере
Процедура КонтрагентыПриСозданииНаСервере()
	
	Если Пользователи.РолиДоступны("а_ЧтениеДобавлениеИзменениеДанныхБухгалтерии",,) Тогда
		// паспортные данные
		тз = РегистрыСведений.а_ПаспортныеДанные.СрезПоследних(, Новый Структура("ФизЛицо", Объект.Ссылка));
		Если тз.Количество() Тогда
			Элементы.а_ПаспортныеДанные.Заголовок = СтрШаблон("%1 %2 %3 от %4",
				тз[0].ДокументВид,
				тз[0].ДокументСерия,
				тз[0].ДокументНомер,
				Формат(тз[0].ДокументДатаВыдачи, "ДФ=dd.MM.yyyyр.")
			);
		КонецЕсли;
	
	Иначе
		Элементы.Группа2.Видимость = Ложь;//Объект.ЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо");
	КонецЕсли;
...
&НаКлиенте
Процедура а_ПаспортныеДанныеНажатие(Элемент)
	
	ОткрытьФорму("РегистрСведений.а_ПаспортныеДанные.ФормаСписка", 
		Новый Структура("Отбор", 
			Новый Структура("ФизЛицо", Объект.Ссылка)
		)
	);
