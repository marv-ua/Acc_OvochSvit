...
Функция ТоварыПоДаннымПоступления(ДанныеОбъекта, ДокументПоступления, НомерРаздела, СтавкаНДС) Экспорт
	...
	Для каждого СтрокаТоваров Из ТаблицаЗначенийТовары Цикл
		...
		СчетаСУчетомПоступления = Новый Структура("СчетУчетаБУ, НалоговоеНазначение");
		Для каждого ЭлементСтруктуры Из СчетаСУчетомПоступления Цикл
			СчетаСУчетомПоступления[ЭлементСтруктуры.Ключ] = ?(ЗначениеЗаполнено(СтрокаТоваров[ЭлементСтруктуры.Ключ]), СтрокаТоваров[ЭлементСтруктуры.Ключ], СчетаУчета[ЭлементСтруктуры.Ключ]);
		КонецЦикла;

		Документы.ГТДИмпорт.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(
			ДанныеОбъекта, СтрокаТоваров, "Товары", СчетаСУчетомПоступления, СтавкаНДС);
