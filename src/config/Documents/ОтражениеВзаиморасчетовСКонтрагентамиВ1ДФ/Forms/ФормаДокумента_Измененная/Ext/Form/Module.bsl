...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Test{20231101
	НоваяКоманда = Команды.Добавить("КомандаЗаполнитьПоПоступлениям");
	НоваяКоманда.Действие = "ЗаполнитьПоПоступлениям";//Имя процедуры
	НоваяКоманда.Заголовок = "По надходженням";

	НовыйЭлемент = Элементы.Добавить("ЭлементЗаполнитьПоПоступлениям",
	                            Тип("КнопкаФормы"),
								Элементы.ГруппаЗаполнить);
	НовыйЭлемент.ИмяКоманды = "КомандаЗаполнитьПоПоступлениям";

	нЭлемент = Элементы.Добавить("а_ПеречисленныйДоход", Тип("ПолеФормы"), Элементы.НДФЛ); 
	нЭлемент.Вид = ВидПоляФормы.ПолеВвода; 
	нЭлемент.ПутьКДанным = "Объект.НДФЛ.а_ПеречисленныйДоход"; 
	// Test}
КонецПроцедуры
...
Процедура ЗаполнитьПоПоступлениямСервер()
	...
	Запрос.Текст = 
		...
		|	СУММА(Т.Доход) КАК Доход,
		|	СУММА(Т.УдержНДФЛ) КАК УдержНДФЛ
		|ПОМЕСТИТЬ ВТДок
		|ИЗ
		|	ВТ_Док КАК Т
		|
		...
		|	СУММА(ХозрасчетныйОборотыДтКт.СуммаОборот) КАК а_ПеречисленныйДоход
		|ИЗ
		|	ВТДок КАК ВТ_Док
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, Месяц, СчетДт.Код В (""6851"", ""631""), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты), СчетКт.Код В (""311""), , ) КАК ХозрасчетныйОборотыДтКт
		|		ПО ВТ_Док.Контрагент = ХозрасчетныйОборотыДтКт.СубконтоДт1
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_Док.Контрагент,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КонтрагентКодПДФО = Справочники.ВидыДоходовНДФЛ.Код42 Или Не ЗначениеЗаполнено(Выборка.КонтрагентКодПДФО) Тогда
			...
			//ТекСтрока.Налог = Выборка.УдержНДФЛ;
			// Test{20230927
			ЗаполнитьЗначенияСвойств(ТекСтрока, Выборка, "а_ПеречисленныйДоход");
			// Test}
		Иначе
