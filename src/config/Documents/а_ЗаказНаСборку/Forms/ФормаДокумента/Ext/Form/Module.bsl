...
Функция ПечатьБиркиНаСервере()
	...
	Для Каждого Область Из МассивОбластей Цикл
		...
		Если ((ВіводСтрокиБирки) % штПоВысоте) = 0 И ВіводСтрокиБирки <> 0 Тогда
		...
		Если СчВертикальныйСдвигов >= штПоШирине-1 Тогда
			...
			ВіводСтрокиБирки = ВіводСтрокиБирки + 1;
		Иначе
...
Процедура УстановитьДоступностьКнопкиСобран()
	...
	Элементы.ФормаСобран.Доступность = ВсеСобраны И Объект.ОзнакомленСПримечаниями;
...
Процедура ОбновитьТаблицуМаршрут()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ КАК Текущий,
	               |	а_ЗаказНаСборкуТовары.Ссылка.ПунктРазгрузки.НаименованиеПолное КАК ПунктРазгрузки
	               |ИЗ
	               ...
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ,
	               |	а_ЗаказНаСборкуТовары.Ссылка.ПунктРазгрузки.НаименованиеПолное";
...
&НаКлиенте
Процедура ОзнакомленСПримечаниямиПриИзменении(Элемент)
	УстановитьДоступностьКнопкиСобран();
КонецПроцедуры
