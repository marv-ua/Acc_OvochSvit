...
Процедура ОбновитьСписокПодбора() 
	...
	Если СписокПлатежныхПоручений.Количество() = 0 Тогда
		Если ЗначениеЗаполнено(ОтборБанковскийСчет) Тогда
			ТекстПодсказки = НСтр("ru='С %1 по %2 нет неоплаченных документов по организации %3 и счету в банке ""%4""';uk='З %1 по %2 немає неоплачених документів по організації %3 та рахунку в банку ""%4""'");
		Иначе
