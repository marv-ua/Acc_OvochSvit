...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	// Test{20240131
	МассивСтрок = Список.НайтиСтроки(Новый Структура("Пометка", Истина));
	Если МассивСтрок.Количество() Тогда
		МассивДокументов = Новый Массив;
		Для Каждого Эл Из МассивСтрок Цикл
			Если ЗначениеЗаполнено(Эл.Документ) Тогда
				МассивДокументов.Добавить(Эл.Документ);
			КонецЕсли;
		КонецЦикла;
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	// Test{20240131 
	Если Элемент.ТекущийЭлемент = Элементы.СписокПометка Тогда
		Возврат;
	КонецЕсли;
	// Test}
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Пометка", Новый ОписаниеТипов("Булево"), "Список", " "));
	МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("СуммаВыбранных", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(20,2)), "", "Сума обраних"));
	ИзменитьРеквизиты(МассивДобавляемыхРеквизитов);
	
	а_ПрограмнаяРаботаСФормами.РазместитьРеквизитНаФорме(ЭтаФорма, "СуммаВыбранных",, "СуммаВыбранных",,,, Истина,,"ГруппаДокументыРасчетовСКонтрагентами",,);
	Элементы.СуммаВыбранных.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	Элементы.СуммаВыбранных.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Право;
	
	а_ПрограмнаяРаботаСФормами.РазместитьРеквизитНаФорме(ЭтаФорма,
		"СписокПометка",
		"Список",
		"Список.Пометка",
		,
		...
		,
		"СписокСумма",
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Новый Структура("Событие,Действие", "ПриИзменении", "ПометкаПриИзменении")),
		" "
	);
	Элементы.СписокПометка.Вид = ВидПоляФормы.ПолеФлажка;
	// Test}
...
Процедура ПометкаПриИзменении(Элемент)
	...
	Для Каждого Эл Из Список Цикл
		Если Эл.Пометка Тогда
			СуммаВыбранных = СуммаВыбранных + Эл.Сумма;
		КонецЕсли;
	КонецЦикла;
