...
// ПРОЦЕДУРЫ И ФУНКЦИИ ПО ФОРМИРОВАНИЮ НАЛОГОВЫХ РАЗНИЦ ПРИ ПЕРЕХОДЕ С ЕН НА ПРИБЫЛЬ
Процедура ФормированиеНалоговыхРазниц(СтруктураШапкиДокумента, Отказ, Заголовок)
	
	Если НЕ СтруктураШапкиДокумента.РасчетНалоговыхРазниц Тогда
		Возврат;
	КонецЕсли;
	
	ВестиРасчетыПоДокументам = ?(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами, "ВидСубконто") = Неопределено, Ложь, Истина);	
    НаборЗаписей = Движения.ОжидаемыеНалоговыеРазницы;
	
	//общие параметры запросов
	Запрос = Новый Запрос;

	Запрос.УстановитьПараметр("НачалоМесяца", НачалоМесяца(СтруктураШапкиДокумента.Дата));
	Запрос.УстановитьПараметр("КонецМесяца", КонецМесяца(СтруктураШапкиДокумента.Дата));
	Запрос.УстановитьПараметр("ПериодОстатков", МоментВремени());
	
	Запрос.УстановитьПараметр("Организация", СтруктураШапкиДокумента.Организация);
	Запрос.УстановитьПараметр("ГРН", Константы.ВалютаРегламентированногоУчета.Получить());
	
	ГруппыСчетовУчета = ПолучитьДетальныеСпискиСчетовУчета();
	Для каждого ГруппаСчетов ИЗ ГруппыСчетовУчета Цикл
		Запрос.УстановитьПараметр(ГруппаСчетов.Ключ, ГруппаСчетов.Значение);
	КонецЦикла;
	...
	// 1. Формирование разниц в первом месяце после переходана прибыль
	Если СтруктураШапкиДокумента.ЕстьНалогНаПрибыль И 	
		УчетнаяПолитика.ПлательщикЕдиногоНалога(СтруктураШапкиДокумента.Организация, НачалоМесяца(СтруктураШапкиДокумента.Дата) - 1) Тогда
		
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СтатусыПлательщиковЕдиногоНалога.Период КАК Период,
		|	СтатусыПлательщиковЕдиногоНалога.ПлательщикЕдиногоНалога КАК ПлательщикЕдиногоНалога
		|ИЗ
		...
		|	И СтатусыПлательщиковЕдиногоНалога.Период < &КонецМесяца
		|
		|УПОРЯДОЧИТЬ ПО
		|	Период УБЫВ";
		
		НачалоПериодаЕН = Неопределено;
		КонецПериодаЕН = Неопределено;
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			
			Если НЕ Выборка.ПлательщикЕдиногоНалога Тогда
				Если НачалоПериодаЕН = Неопределено Тогда
					КонецПериодаЕН = Выборка.Период - 1;
				Иначе
					Прервать;
				КонецЕсли;
			ИначеЕсли КонецПериодаЕН <> Неопределено Тогда 
				НачалоПериодаЕН = Выборка.Период;
			КонецЕсли;
			
		КонецЦикла;
		
		Если НачалоПериодаЕН <> Неопределено И КонецПериодаЕН <> Неопределено Тогда
			
			Запрос.УстановитьПараметр("НачалоПериодаЕН", НачалоПериодаЕН);
			Запрос.УстановитьПараметр("КонецПериодаЕН", КонецПериодаЕН);
			Запрос.УстановитьПараметр("КонецПериодаЕНВключая", Новый Граница(КонецПериодаЕН, ВидГраницы.Включая));

			//распределение остатков расчетов на конец периода ЕН по образовавшим их регистраторам
			Если ВестиРасчетыПоДокументам Тогда
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	ХозрасчетныйОстатки.Счет КАК Счет,
				...
				|	ЕСТЬNULL(ХозрасчетныйОстатки.Валюта, &ГРН) КАК Валюта,
				|	ВЫБОР
				...
				|	АВТОНОМЕРЗАПИСИ() КАК НомерАналитики
				|ПОМЕСТИТЬ Остатки
				|ИЗ
				|	РегистрБухгалтерии.Хозрасчетный.Остатки(&КонецПериодаЕНВключая, Счет В (&СчетаРасчетов), &ВидыСубконто, Организация = &Организация) КАК ХозрасчетныйОстатки
				|ГДЕ
				|	(ВЫБОР
				|				КОГДА ХозрасчетныйОстатки.Валюта ЕСТЬ NULL
				|					ТОГДА ХозрасчетныйОстатки.СуммаОстатокДт
				...
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	ХозрасчетныйОбороты.Регистратор КАК ДокументДвижения,
				|	ХозрасчетныйОбороты.Период КАК ПериодДвижения,
				|	ХозрасчетныйОбороты.Счет КАК Счет,
				|	ХозрасчетныйОбороты.Субконто1 КАК Контрагент,
				|	ХозрасчетныйОбороты.Субконто2 КАК Договор,
				|	ХозрасчетныйОбороты.Субконто3 КАК ДокументРасчетов,
				|	ЕСТЬNULL(ХозрасчетныйОбороты.Валюта, &ГРН) КАК Валюта,
				|	ВЫБОР
				...
				|	Остатки.НомерАналитики КАК НомерАналитики
				|ИЗ
				|	РегистрБухгалтерии.Хозрасчетный.Обороты(
				|			&НачалоПериодаЕН,
				|			&КонецПериодаЕН,
				...
				|		ПО ХозрасчетныйОбороты.Счет = Остатки.Счет
				|			И (ЕСТЬNULL(ХозрасчетныйОбороты.Валюта, &ГРН) = Остатки.Валюта)
				|			И ХозрасчетныйОбороты.Субконто1 = Остатки.Субконто1
				|			И ХозрасчетныйОбороты.Субконто2 = Остатки.Субконто2
				|			И ХозрасчетныйОбороты.Субконто3 = Остатки.Субконто3
				|			И (Остатки.СуммаОстатокДт > 0
				|					И ХозрасчетныйОбороты.СуммаОборотДт > 0
				|					И ХозрасчетныйОбороты.КорСчет В (&СчетаДоходов)
				|				ИЛИ Остатки.СуммаОстатокКт > 0
				|					И ХозрасчетныйОбороты.СуммаОборотКт > 0
				|					И ХозрасчетныйОбороты.КорСчет В (&СчетаДС))
				|			И (НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПередачаОС)
				|			И (НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПередачаНМА)
				|
				|УПОРЯДОЧИТЬ ПО
				|	НомерАналитики,
			...
			ТаблицаОжидаемыхРазниц = НаборЗаписей.ВыгрузитьКолонки();
			
			РезультатЗапроса = Запрос.Выполнить();
			ВыборкаАналитика = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаАналитика.Следующий() Цикл
				
				Если ЗначениеЗаполнено(ВыборкаАналитика.СуммаОстатокДт) Тогда
					СуммаКРаспределению = ВыборкаАналитика.СуммаОстатокДт;
					ЭтоАванс = Ложь;
				Иначе
					СуммаКРаспределению = ВыборкаАналитика.СуммаОстатокКт;
					ЭтоАванс = Истина;
				КонецЕсли;           
				...
				Пока Выборка.Следующий() Цикл
					...
					СуммаПоДокументу = Макс(Мин(СуммаКРаспределению, ?(ЭтоАванс, Выборка.СуммаОборотКт, Выборка.СуммаОборотДт)), 0);
					
					Если ЗначениеЗаполнено(СуммаПоДокументу) Тогда
						
						НоваяСтрока = ТаблицаОжидаемыхРазниц.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
						НоваяСтрока.Организация = СтруктураШапкиДокумента.Организация;
						НоваяСтрока.СуммаВзаиморасчетов = СуммаПоДокументу;
						НоваяСтрока.Аванс = ЭтоАванс;
						Если НЕ ЭтоАванс Тогда

							СуммыДокументаДвижения = ПолучитьФинансовыеПоказателиДокумента(НоваяСтрока.ДокументДвижения, ГруппыСчетовУчета);
							
							Если СуммыДокументаДвижения.СуммаВзаиморасчетов <> 0 Тогда
								
								Коэф = СуммаПоДокументу / СуммыДокументаДвижения.СуммаВзаиморасчетов;
								НоваяСтрока.СуммаДоходов = СуммыДокументаДвижения.СуммаДоходов * Коэф;
								НоваяСтрока.Себестоимость = СуммыДокументаДвижения.Себестоимость * Коэф;
								
							КонецЕсли;

						КонецЕсли;                   
						
						СуммаКРаспределению = СуммаКРаспределению - СуммаПоДокументу;
			...
			НаборЗаписей.мПериод = НачалоМесяца(СтруктураШапкиДокумента.Дата);
			НаборЗаписей.мТаблицаДвижений = ТаблицаОжидаемыхРазниц;
	...
	// 2. Списание ожидаемых налоговых разниц по свершившимся вторым событиям
	Если ВестиРасчетыПоДокументам Тогда
		
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОжидаемыеНалоговыеРазницыОстатки.Счет КАК Счет,
		|	ОжидаемыеНалоговыеРазницыОстатки.Контрагент КАК Субконто1,
		|	ОжидаемыеНалоговыеРазницыОстатки.Договор КАК Субконто2,
		|	ОжидаемыеНалоговыеРазницыОстатки.ДокументРасчетов КАК Субконто3,
		|	ОжидаемыеНалоговыеРазницыОстатки.Валюта КАК Валюта,
		|	ОжидаемыеНалоговыеРазницыОстатки.Аванс КАК Аванс
		|ПОМЕСТИТЬ Остатки
		|ИЗ
		|	РегистрНакопления.ОжидаемыеНалоговыеРазницы.Остатки(&ПериодОстатков, Организация = &Организация) КАК ОжидаемыеНалоговыеРазницыОстатки
		|ГДЕ
		|	ОжидаемыеНалоговыеРазницыОстатки.СуммаВзаиморасчетовОстаток > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|					ИНАЧЕ ХозрасчетныйОбороты.ВалютнаяСуммаОборотКт
		|				КОНЕЦ
		|		КОНЕЦ) КАК СуммаВзаиморасчетовСписания,
		|	СУММА(ВЫБОР
		|			КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПередачаОС
		|					ИЛИ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПередачаНМА
		|				ТОГДА 0
		|			КОГДА Остатки.Аванс
		|					И ХозрасчетныйОбороты.КорСчет В (&СчетаДоходов, &СчетаРасчетов)
		|				ТОГДА ВЫБОР
		|						КОГДА ХозрасчетныйОбороты.Валюта ЕСТЬ NULL
		|							ТОГДА ХозрасчетныйОбороты.СуммаОборотДт
		|						ИНАЧЕ ХозрасчетныйОбороты.ВалютнаяСуммаОборотДт
		|					КОНЕЦ
		|			КОГДА НЕ Остатки.Аванс
		|					И ХозрасчетныйОбороты.КорСчет В (&СчетаДС)
		|				ТОГДА ВЫБОР
		|						КОГДА ХозрасчетныйОбороты.Валюта ЕСТЬ NULL
		|							ТОГДА ХозрасчетныйОбороты.СуммаОборотКт
		...
		|					КОНЕЦ
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК СуммаВзаиморасчетовРИ,
		|	Остатки.Аванс КАК Аванс
		|ПОМЕСТИТЬ СписанияИзОстатков
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Обороты(
		|			&НачалоМесяца,
		|			&КонецМесяца,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СписанияИзОстатков.Счет КАК Счет,
		|	СписанияИзОстатков.Валюта КАК Валюта,
		|	СписанияИзОстатков.Контрагент КАК Контрагент,
		|	СписанияИзОстатков.Договор КАК Договор,
		|	СписанияИзОстатков.ДокументРасчетов КАК ДокументРасчетов,
		|	СписанияИзОстатков.Аванс КАК Аванс,
		|	АВТОНОМЕРЗАПИСИ() КАК НомерАналитики
		|ПОМЕСТИТЬ Аналитика
		|ИЗ
		|	СписанияИзОстатков КАК СписанияИзОстатков
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Аналитика.НомерАналитики КАК НомерАналитики,
		|	1 КАК Раздел,
		|	ОжидаемыеНалоговыеРазницыОстатки.ПериодДвижения КАК ПериодДвижения,
		|	ОжидаемыеНалоговыеРазницыОстатки.ДокументДвижения КАК ДокументДвижения,
		|	ОжидаемыеНалоговыеРазницыОстатки.Счет КАК Счет,
		|	ОжидаемыеНалоговыеРазницыОстатки.Валюта КАК Валюта,
		|	ОжидаемыеНалоговыеРазницыОстатки.Контрагент КАК Контрагент,
		|	ОжидаемыеНалоговыеРазницыОстатки.Договор КАК Договор,
		|	ОжидаемыеНалоговыеРазницыОстатки.ДокументРасчетов КАК ДокументРасчетов,
		|	ОжидаемыеНалоговыеРазницыОстатки.СуммаВзаиморасчетовОстаток КАК СуммаВзаиморасчетов,
		|	ОжидаемыеНалоговыеРазницыОстатки.Аванс КАК Аванс,
		|	ОжидаемыеНалоговыеРазницыОстатки.СуммаДоходовОстаток КАК СуммаДоходов,
		|	ОжидаемыеНалоговыеРазницыОстатки.СебестоимостьОстаток КАК Себестоимость,
		|	0 КАК СуммаВзаиморасчетовРИ
		|ИЗ
		|	РегистрНакопления.ОжидаемыеНалоговыеРазницы.Остатки(
		|			&ПериодОстатков,
		|			Организация = &Организация
		|				И (Счет, Валюта, Контрагент, Договор, ДокументРасчетов, Аванс) В
		|					(ВЫБРАТЬ
		|						Т.Счет,
		|						Т.Валюта,
		|						Т.Контрагент,
		...
		|						Т.Аванс
		|					ИЗ
		|						Аналитика КАК Т)) КАК ОжидаемыеНалоговыеРазницыОстатки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Аналитика КАК Аналитика
		|		ПО ОжидаемыеНалоговыеРазницыОстатки.Счет = Аналитика.Счет
		|			И ОжидаемыеНалоговыеРазницыОстатки.Валюта = Аналитика.Валюта
		|			И ОжидаемыеНалоговыеРазницыОстатки.Контрагент = Аналитика.Контрагент
		|			И ОжидаемыеНалоговыеРазницыОстатки.Договор = Аналитика.Договор
		|			И ОжидаемыеНалоговыеРазницыОстатки.ДокументРасчетов = Аналитика.ДокументРасчетов
		|			И ОжидаемыеНалоговыеРазницыОстатки.Аванс = Аналитика.Аванс
		|ГДЕ
		|	ОжидаемыеНалоговыеРазницыОстатки.СуммаВзаиморасчетовОстаток > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	Аналитика.НомерАналитики,
		|	2,
		|	СписанияИзОстатков.Период,
		|	СписанияИзОстатков.Регистратор,
		|	СписанияИзОстатков.Счет,
		|	СписанияИзОстатков.Валюта,
		|	СписанияИзОстатков.Контрагент,
		|	СписанияИзОстатков.Договор,
		|	СписанияИзОстатков.ДокументРасчетов,
		|	СписанияИзОстатков.СуммаВзаиморасчетовСписания,
		|	СписанияИзОстатков.Аванс,
		|	0,
		|	0,
		|	СписанияИзОстатков.СуммаВзаиморасчетовРИ
		|ИЗ
		|	СписанияИзОстатков КАК СписанияИзОстатков
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Аналитика КАК Аналитика
		|		ПО СписанияИзОстатков.Счет = Аналитика.Счет
		|			И СписанияИзОстатков.Валюта = Аналитика.Валюта
		|			И СписанияИзОстатков.Контрагент = Аналитика.Контрагент
		|			И СписанияИзОстатков.Договор = Аналитика.Договор
		|			И СписанияИзОстатков.ДокументРасчетов = Аналитика.ДокументРасчетов
		|			И СписанияИзОстатков.Аванс = Аналитика.Аванс
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерАналитики,
		|	Раздел,
	...
	Иначе
		...
		|			&ПериодОстатков,
		|			Организация = &Организация
		|				И (Счет, Валюта, Контрагент, Договор, Аванс) В
		|					(ВЫБРАТЬ
		|						Т.Счет,
		|						Т.Валюта,
		|						Т.Контрагент,
	...
	ТаблицаОстатков.Колонки.Добавить("ПериодДвижения");
	ТаблицаОстатков.Колонки.Добавить("ДокументДвижения");
	ТаблицаОстатков.Колонки.Добавить("СуммаВзаиморасчетов");
	ТаблицаОстатков.Колонки.Добавить("СуммаДоходов");
	ТаблицаОстатков.Колонки.Добавить("Себестоимость");
	
	ТаблицаОжидаемыхРазниц = НаборЗаписей.ВыгрузитьКолонки();
	КэшФинансовыхПоказателей = Новый Соответствие;
	СуммаРИОтгрузкаПослеОплаты = 0;
	СуммаРИОплатаПослеОтгрузки = 0;
	...
	Пока ВыборкаАналитика.Следующий() Цикл
		...
		ВыборкаРаздел = ВыборкаАналитика.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаРаздел.Следующий() Цикл
			...
			Иначе
				...
				Пока Выборка.Следующий() Цикл
					...
					СуммаКРаспределению = Выборка.СуммаВзаиморасчетов;
					КоэфРИ = Выборка.СуммаВзаиморасчетовРИ / Выборка.СуммаВзаиморасчетов;
					...
					Для каждого СтрокаОстатков Из ТаблицаОстатков Цикл
						...
						СуммаПоСтроке = Мин(СуммаКРаспределению, СтрокаОстатков.СуммаВзаиморасчетов);
						
						Если ЗначениеЗаполнено(СуммаПоСтроке) Тогда
							
							НоваяСтрока = ТаблицаОжидаемыхРазниц.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
							ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОстатков, "ПериодДвижения, ДокументДвижения");
							НоваяСтрока.Организация = СтруктураШапкиДокумента.Организация;
							НоваяСтрока.ДокументВторогоСобытия = Выборка.ДокументДвижения;
							НоваяСтрока.СуммаВзаиморасчетов = СуммаПоСтроке;
							
							Если Выборка.Аванс И КоэфРИ <> 0 Тогда
								
								//закрываем аванс реализацией текущего периода, финансовые показатели влияют только на сумму РИ
								ФинансовыеПоказателиДокумента = КэшФинансовыхПоказателей[Выборка.ДокументДвижения];
								Если ФинансовыеПоказателиДокумента = Неопределено Тогда
									
									ФинансовыеПоказателиДокумента = ПолучитьФинансовыеПоказателиДокумента(Выборка.ДокументДвижения, ГруппыСчетовУчета);
									КэшФинансовыхПоказателей.Вставить(Выборка.ДокументДвижения, ФинансовыеПоказателиДокумента);
									
								КонецЕсли;
								
								Если ФинансовыеПоказателиДокумента.СуммаВзаиморасчетов <> 0 Тогда
									
									Коэф = СуммаПоСтроке / ФинансовыеПоказателиДокумента.СуммаВзаиморасчетов;
									СуммаДоходов = ФинансовыеПоказателиДокумента.СуммаДоходов * Коэф;
									Себестоимость = ФинансовыеПоказателиДокумента.Себестоимость * Коэф;
									
									НоваяСтрока.СуммаДоходовВтороеСобытие = СуммаДоходов;
									НоваяСтрока.СебестоимостьВтороеСобытие = Себестоимость;
									
									НоваяСтрока.СуммаРИ = (СуммаДоходов - Себестоимость) * КоэфРИ;
									
									СуммаРИОтгрузкаПослеОплаты = СуммаРИОтгрузкаПослеОплаты + НоваяСтрока.СуммаРИ; 
									
								КонецЕсли;
								
							Иначе
								
								//реализация закрывается оплатой, финансовые показатели сохранены в остатках
								Коэф = СуммаПоСтроке / СтрокаОстатков.СуммаВзаиморасчетов;
								НоваяСтрока.СуммаДоходов = СтрокаОстатков.СуммаДоходов * Коэф;
								НоваяСтрока.Себестоимость = СтрокаОстатков.Себестоимость * Коэф;
								
								СтрокаОстатков.СуммаДоходов = СтрокаОстатков.СуммаДоходов - НоваяСтрока.СуммаДоходов;
								СтрокаОстатков.Себестоимость = СтрокаОстатков.Себестоимость - НоваяСтрока.Себестоимость;
								
								НоваяСтрока.СуммаРИ = (НоваяСтрока.СуммаДоходов - НоваяСтрока.Себестоимость) * КоэфРИ;
								
								СуммаРИОплатаПослеОтгрузки = СуммаРИОплатаПослеОтгрузки + НоваяСтрока.СуммаРИ;
								
							КонецЕсли;
							
							СуммаКРаспределению = СуммаКРаспределению - СуммаПоСтроке;
							СтрокаОстатков.СуммаВзаиморасчетов = СтрокаОстатков.СуммаВзаиморасчетов - СуммаПоСтроке;
	...
	Если ТаблицаОжидаемыхРазниц.Количество() > 0 Тогда
	...
	Если ЗначениеЗаполнено(СуммаРИОтгрузкаПослеОплаты) Тогда
		...
		ПроводкаБУ.СубконтоДт.СтатьиНалоговыхДеклараций = Справочники.СтатьиНалоговыхДеклараций.НП15_РИ_Др_Переходные_КассовыйЗатраты;
		
		ПроводкаБУ.Сумма                           = СуммаРИОтгрузкаПослеОплаты;
		
	КонецЕсли;

	Если ЗначениеЗаполнено(СуммаРИОплатаПослеОтгрузки) Тогда
		...
		ПроводкаБУ.СчетДт                          = СчетРИ;
		ПроводкаБУ.СубконтоДт.СтатьиНалоговыхДеклараций = Справочники.СтатьиНалоговыхДеклараций.НП15_РИ_Фин_ОплатаТоваров_ЕН;
		
		ПроводкаБУ.Сумма                           = СуммаРИОплатаПослеОтгрузки;
...
Функция ПолучитьФинансовыеПоказателиДокумента(ДокументСсылка, ГруппыСчетовУчета = Неопределено)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(ВЗ.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов,
	|	СУММА(ВЗ.СуммаДоходов) КАК СуммаДоходов,
	|	СУММА(ВЗ.Себестоимость) КАК Себестоимость
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА Хозрасчетный.СчетДт В (&СчетаРасчетов)
	|				ТОГДА ВЫБОР КОГДА Хозрасчетный.ВалютаДт IS NULL ТОГДА Хозрасчетный.Сумма ИНАЧЕ Хозрасчетный.ВалютнаяСуммаДт КОНЕЦ
	|			ИНАЧЕ 0
	|		КОНЕЦ - ВЫБОР
	|			КОГДА Хозрасчетный.СчетКт В (&СчетаРасчетов)
	|				ТОГДА ВЫБОР КОГДА Хозрасчетный.ВалютаКт IS NULL ТОГДА Хозрасчетный.Сумма ИНАЧЕ Хозрасчетный.ВалютнаяСуммаКт КОНЕЦ
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК СуммаВзаиморасчетов,
	|		ВЫБОР
	|			КОГДА Хозрасчетный.СчетДт В (&СчетаДоходов)
	|				ТОГДА -Хозрасчетный.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ + ВЫБОР
	|			КОГДА Хозрасчетный.СчетКт В (&СчетаДоходов)
	|				ТОГДА Хозрасчетный.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК СуммаДоходов,
	|		ВЫБОР
	|			КОГДА Хозрасчетный.СчетДт В (&СчетаСебестоимости)
	|				ТОГДА Хозрасчетный.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ - ВЫБОР
	|			КОГДА Хозрасчетный.СчетКт В (&СчетаСебестоимости)
	|				ТОГДА Хозрасчетный.Сумма
	|			ИНАЧЕ 0
	...
	|	ГДЕ
	|		Хозрасчетный.Регистратор = &Регистратор) КАК ВЗ";
	Запрос.УстановитьПараметр("Регистратор", ДокументСсылка);
	
	Если ГруппыСчетовУчета = Неопределено Тогда
		ГруппыСчетовУчета = ПолучитьДетальныеСпискиСчетовУчета();
	КонецЕсли;
	...
	СтруктураВозврата = Новый Структура("СуммаВзаиморасчетов, СуммаДоходов, Себестоимость", 0, 0, 0);
...
Функция ПолучитьДетальныеСпискиСчетовУчета()
	
	ГруппыСчетовУчета = Новый Структура;
	
	МассивСчетовРасчетов = Новый Массив;
	МассивСчетовРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками);
	МассивСчетовРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным);
	МассивСчетовРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСДругимиДебиторами);
	МассивСчетовРасчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСДругимиКредиторами);
	ГруппыСчетовУчета.Вставить("СчетаРасчетов", МассивСчетовРасчетов);
	
	МассивСчетовДенежныхСредств = Новый Массив;
	МассивСчетовДенежныхСредств.Добавить(ПланыСчетов.Хозрасчетный.Касса);
	МассивСчетовДенежныхСредств.Добавить(ПланыСчетов.Хозрасчетный.СчетаВБанках);
	ГруппыСчетовУчета.Вставить("СчетаДС", МассивСчетовДенежныхСредств);
	
	МассивСчетовДоходов = Новый Массив;
	МассивСчетовДоходов.Добавить(ПланыСчетов.Хозрасчетный.ДоходыОтРеализации);
	МассивСчетовДоходов.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииДругихОборотныхАктивов);
	ГруппыСчетовУчета.Вставить("СчетаДоходов", МассивСчетовДоходов);
	
	МассивСчетовСебестоимости = Новый Массив;
	МассивСчетовСебестоимости.Добавить(ПланыСчетов.Хозрасчетный.СебестоимостьРеализации);
	МассивСчетовСебестоимости.Добавить(ПланыСчетов.Хозрасчетный.СебестоимостьРеализованныхПроизводственныхЗапасов);
	ГруппыСчетовУчета.Вставить("СчетаСебестоимости", МассивСчетовСебестоимости);
	...
	Для каждого ГруппаСчетов ИЗ ГруппыСчетовУчета Цикл
		...
		Запрос.УстановитьПараметр("МассивСчетов", ГруппаСчетов.Значение);
		ГруппыСчетовУчета.Вставить(ГруппаСчетов.Ключ, Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0));
		
	КонецЦикла; 
	
	Возврат ГруппыСчетовУчета;
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	ФормированиеНалоговыхРазниц(СтруктураШапкиДокумента, Отказ, Заголовок);
