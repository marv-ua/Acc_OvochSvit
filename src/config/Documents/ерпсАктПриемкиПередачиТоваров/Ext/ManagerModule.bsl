...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Данные.ПунктРазгрузки) Тогда
		//Заявка актів приймання передачі товарів 000009895 від 12.01.2024 00:00:00											
		Представление = СтрШаблон("%1 (%2) %3 від %4",
			"Акт приймання передачі товарів",
			СокрЛП(Данные.ПунктРазгрузки.Наименование),
			Данные.Номер,
			Формат(Данные.Дата, "ДЛФ=DT")
		); 
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
