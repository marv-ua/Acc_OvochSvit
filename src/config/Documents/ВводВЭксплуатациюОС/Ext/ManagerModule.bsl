...
Функция ПечатьОЗ1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.а_ПФ_MXL_UK_ОЗ1");
	// Test}	
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		Пока ВыборкаПоОС.Следующий() Цикл
			...
			// Test{20231101
			ОтвЛица = ОтветственныеЛицаБП.ОтветственныеЛица(Ссылка.Организация, Ссылка.Дата);
			ОбластьМакета.Параметры.ДиректорФИО = ОтвЛица.РуководительПредставление;
			
			Приказ = ОбщегоНазначенияСервер.ПриказОСНМА(Ссылка);
			Если ЗначениеЗаполнено(Приказ) Тогда
				ОбластьМакета.Параметры.Приказ = "НАКАЗ № "+ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Приказ.Номер, Истина);
				ОбластьМакета.Параметры.прД = День(Приказ.Дата);
				ОбластьМакета.Параметры.прМ = ОбщегоНазначенияСервер.МесяцСтрокойДП(Месяц(Приказ.Дата));
				ОбластьМакета.Параметры.прГ = Формат(Год(Приказ.Дата), "ЧГ=");
			КонецЕсли;

			Подразделение = Ссылка.ПодразделениеОрганизации;
