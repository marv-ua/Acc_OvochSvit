...
Процедура СоздатьЗаявкиНаСервере(Успешно)
	...
	Для Каждого СтрокаРодитель Из тзРодитель Цикл
		...
		Запрос.Текст = "ВЫБРАТЬ
		               |	ерпсЗаявкаНаСозданиеАктовПриемкиПередачи.Ссылка КАК Ссылка
		               |ИЗ
		               |	Документ.ерпсЗаявкаНаСозданиеАктовПриемкиПередачи КАК ерпсЗаявкаНаСозданиеАктовПриемкиПередачи
		               |ГДЕ
		               |	НЕ ерпсЗаявкаНаСозданиеАктовПриемкиПередачи.ПометкаУдаления
		               |	И НАЧАЛОПЕРИОДА(ерпсЗаявкаНаСозданиеАктовПриемкиПередачи.Дата, ДЕНЬ) = НАЧАЛОПЕРИОДА(&Дата, ДЕНЬ)";
...
Процедура ТЧТоварыУдалитьНаСервере(ПунктРазгрузки, Дата)
	...
	|ИЗ ВТТаб КАК Т
	|ГДЕ
	|	Т.ПунктРазгрузки <> &ПунктРазгрузки
	|	И Т.ДатаОтгрузки <> &ДатаОтгрузки
	|";
