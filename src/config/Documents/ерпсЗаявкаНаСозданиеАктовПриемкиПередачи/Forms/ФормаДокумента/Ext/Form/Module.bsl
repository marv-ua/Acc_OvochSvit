...
Функция ПодготовитьПараметрыПечати(ОписаниеКоманды)
	...
	Для каждого Элемент из ПараметрКоманды цикл
		...
		ПараметрыПечати.Вставить("ПунктПогрузки"+стрСчетчик, "м.Київ, вул.Малинська 4Є");
...
Функция ПечатьРННаСервере(Водитель = Неопределено, Авто = Неопределено, ПунктРазгрузки = Неопределено)
	...
	ТабличныйДокумент = Документы.РеализацияТоваровУслуг.ПечатьДокументаБезЦен(МассивРН, Новый СписокЗначений, УправлениеПечатью.ПодготовитьСтруктуруПараметровВывода());
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ТабличныйДокумент.АвтоМасштаб = Истина;
	ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
	Если ТабличныйДокумент.ПроверитьВывод(ТабличныйДокумент) Тогда
		ТабличныйДокумент.Вывести(ТабличныйДокумент);
	Иначе
		ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
		ТабличныйДокумент.Вывести(ТабличныйДокумент);
	КонецЕсли;
...
Процедура ТоварыКоличествоПриИзмененииНаСервере()
	...
	Для Каждого Э Из МассивСтрок Цикл
		Т.Удалить(Э);
	КонецЦикла;
	Объект.ПунктыРазгрузки.Загрузить(Т);
...
Процедура ЗаполнитьКонтактнымиДанными(НомерСтроки = Неопределено)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ТекДанные.ПунктРазгрузки) Тогда
			...
			Иначе
				Попытка
					ТекДанные.Контрагент = ТекДанные.ПунктРазгрузки.Контрагент;
					ТекДанные.Договор = ТекДанные.ПунктРазгрузки.ДоговорКонтрагента;
				Исключение
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не удалось подставить контрагента и договор, попробуйте записать докумнет, выйти с него и зайти заново'; uk = 'Не вдалося підставити контрагента та договір, спробуйте записати документ вийти з нього та зайти знов'"));
				КонецПопытки;
			КонецЕсли;
