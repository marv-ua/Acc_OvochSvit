...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СохраненныеНастройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("КомплектДокументовДляПечати", "КомплектДокументовДляПечати", Новый ТаблицаЗначений,, Пользователи.ТекущийПользователь());
	Если СохраненныеНастройки.Количество() Тогда
		КомплектДокументовДляПечати.Загрузить(СохраненныеНастройки);
	Иначе
		Нов = КомплектДокументовДляПечати.Добавить();
		Нов.Документ = "ТТН"; Нов.Количество = 4;
		Нов = КомплектДокументовДляПечати.Добавить();
		Нов.Документ = "Декларація"; Нов.Количество = 1;
		СохранитьКомплектПечатиНаСервере();
	КонецЕсли;
...
Функция ПечатьДекларацииНаСервере(Водитель = Неопределено, Авто = Неопределено, ПунктРазгрузки = Неопределено, ДатаВывоза = Неопределено)
	...
	Для Каждого Ид Из ТЗ Цикл
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ПО (ДопАртикулы.Идентификатор = Т.Идентификатор)
		               |			И (ДопАртикулы.ДополнительныеАртикулы В
		               |				(ВЫБРАТЬ
		               |					ТТ.ДополнительныеАртикулы
		               |				ИЗ
		               |					ВТНоменклатура КАК ТТ))
		               |ГДЕ
		               |	НАЧАЛОПЕРИОДА(Т.Ссылка.Дата, НЕДЕЛЯ) = НАЧАЛОПЕРИОДА(&ДатаВывоза, НЕДЕЛЯ)";
...
Процедура ТоварыПересчетКоличества(Об)
	...
	тзТоварыПункты.Свернуть("ДатаОтгрузки,ИДЗаявкиКлиента,Идентификатор,Пометка,ПунктРазгрузки");
	...
	Если НомерСтроки = Неопределено Тогда
		Для Каждого ТекДанные Из Об.ПунктыРазгрузки Цикл
			Если ЗначениеЗаполнено(ТекДанные.ПунктРазгрузки) Тогда
				...
				Иначе
					ТекДанные.ДатаОтгрузки = ерпсАктПриемкиПередачиСервер.ПолучитьДатуВывоза(Об.Дата, ТекПункт.ДеньВывоза);
				КонецЕсли;
				...
				ТекДанные.ОригиналыСертификатов = ТекДанные.ПунктРазгрузки.НужныОригиналыСертификатов;
			КонецЕсли;
...
Процедура ЗагрузкаИзВнешнегоФайлаНаСервере(АдресВременногоХранилища)
	...
	//	Если Не ЗначениеЗаполнено(НовСтр.ДатаОтгрузки) Тогда
			НовСтр.ДатаОтгрузки = ерпсАктПриемкиПередачиСервер.ПолучитьДатуВывоза(Объект.Дата, НовСтр.ПунктРазгрузки.ДеньВывоза);
	...
	Для Каждого Эл Из Об.ПунктыРазгрузки Цикл
		Если Не ЗначениеЗаполнено(Эл.Организация) Тогда
			...
			Если Не ЗначениеЗаполнено(Эл.Организация) ТОгда
				...
				Эл.Организация = ерпсАктПриемкиПередачиПовтИсп.ОрганизацияПоУмолчанию();
...
Процедура ОбновитьДатуВывозаФакт()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	ВЫБОР
	               |		КОГДА ЕСТЬNULL(РеализацияТоваровУслуг_ТТН.Ссылка, ЛОЖЬ) = ЛОЖЬ
	               |			ТОГДА 1
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК СканТТН,
	               |	ВЫБОР
	               |		КОГДА ЕСТЬNULL(РеализацияТоваровУслуг_РН.Ссылка, ЛОЖЬ) = ЛОЖЬ
	               |			ТОГДА 1
	               |		ИНАЧЕ 0
	               |	КОНЕЦ КАК Скан,
	               |	"" "" КАК Реквизит1
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.а_СтатусыЗаявок.СрезПоследних КАК СтатусыМаршрут
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.а_СтатусыЗаявок.СрезПоследних КАК СтатусыСобран
	               ...
	               |			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РеализацияТоваровУслугПрисоединенныеФайлы КАК РеализацияТоваровУслуг_ТТН
	               |			ПО а_ДокументыРеализацииЗаявок.Реализация = РеализацияТоваровУслуг_ТТН.ВладелецФайла
	               |				И (РеализацияТоваровУслуг_ТТН.Наименование ПОДОБНО ""%ттн_%"")
	               |			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РеализацияТоваровУслугПрисоединенныеФайлы КАК РеализацияТоваровУслуг_РН
	               |			ПО а_ДокументыРеализацииЗаявок.Реализация = РеализацияТоваровУслуг_РН.ВладелецФайла
	               |				И (НЕ РеализацияТоваровУслуг_РН.Наименование ПОДОБНО ""%ттн_%"")
...
Процедура УстановитьНовыйИдентификаторДату(Команда)
	...
	Первый = Истина; текПункт = Неопределено; текДата = Неопределено;
	Для Каждого С Из ВыделенныеСтроки Цикл
		ТекДанные = Объект.Товары.НайтиПоИдентификатору(С);
		Если Не ТекДанные = Неопределено Тогда			
			...
			Если Первый Тогда
				текПункт = ТекДанные.ПунктРазгрузки;
				текДата = ТекДанные.ДатаОтгрузки;
				Первый = Ложь;
			КонецЕсли;
			ТекДанные.ПунктРазгрузки = текПункт;
			ТекДанные.ДатаОтгрузки = текДата;
		КонецЕсли;
	КонецЦикла;
	ТоварыКоличествоПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура СоздатьЗаказНаСборкуНаСервере(Идентификатор, ПунктРазгрузки, ДатаОтгрузки, ЗаказНаЗборку = Неопределено, ОбновлениеДокумента = Ложь)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	ПунктыРазгрузки.ОригиналыСертификатов КАК ОригиналыСертификатов
	               |ИЗ
...
&НаСервере
Функция СоздатьРТУНаСервере(ПунктРазгрузки, Идентификатор, ВыводитьПредупреждения = Истина)
	...
	Если ВыборкаДок.Следующий() Тогда
		...
		Для Каждого СтрокаТоваров Из ТаблицаТоваров Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.СхемаРеализации) Тогда
				СтрокаТабличнойЧасти.СхемаРеализации = Справочники.СхемыРеализации.Товары;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.СчетУчетаБУ) Тогда 
				СтрокаТабличнойЧасти.СчетУчетаБУ = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ТоварыНаСкладе");
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.НалоговоеНазначение) Тогда 
				СтрокаТабличнойЧасти.НалоговоеНазначение = ПредопределенноеЗначение("Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая");
			КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузитьЗаявкуХарьковская(Команда)

	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаЗагрузитьЗаявкуХарьковская_Завершение", ЭтотОбъект)
		, "Очистити документ? Чи додати в поточну заяку?
		|ТАК - Очистити, НІ - додатив поточну"
		, РежимДиалогаВопрос.ДаНет
	);
...
&НаКлиенте
Процедура ЗагрузитьЗаявкуТрейдГранит(Команда)

	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаЗагрузитьЗаявкуТрейдГранит_Завершение", ЭтотОбъект)
		, "Очистити документ? Чи додати в поточну заяку?
		|ТАК - Очистити, НІ - додатив поточну"
		, РежимДиалогаВопрос.ДаНет
	);
...
&НаКлиенте
Процедура ЗагрузитьЗаявкуТоварСверху(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаЗагрузитьТоварСверху_Завершение", ЭтотОбъект)
		, "Очистити документ? Чи додати в поточну заяку?
		|ТАК - Очистити, НІ - додатив поточну"
		, РежимДиалогаВопрос.ДаНет
	);
...
&НаКлиенте
Процедура ПослеВопросаЗагрузитьЗаявкуХарьковская_Завершение(Результат, ДопПараметры) Экспорт
    ...
    ОбработкаОкончанияПомещения = Новый ОписаниеОповещения(
		"ОбработчикОкончанияПомещенияЗаявкиХарьковская"
		, ЭтотОбъект
	);
...
&НаКлиенте
Процедура ПослеВопросаЗагрузитьЗаявкуТрейдГранит_Завершение(Результат, ДопПараметры) Экспорт
    ...
    ОбработкаОкончанияПомещения = Новый ОписаниеОповещения(
		"ОбработчикОкончанияПомещенияЗаявкиТрейдГранит"
		, ЭтотОбъект
	);
...
&НаКлиенте
Процедура ПослеВопросаЗагрузитьТоварСверху_Завершение(Результат, ДопПараметры) Экспорт
    ...
    ОбработкаОкончанияПомещения = Новый ОписаниеОповещения(
		"ОбработчикОкончанияПомещенияЗаявкиТоварСверху"
		, ЭтотОбъект
	);
...
&НаКлиенте
Процедура ОбработчикОкончанияПомещенияЗаявкиХарьковская(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ОбработатьЗаявкуХарьковскаяНаСервере(Адрес
			, УникальныйИдентификатор
			, Сред(ВыбранноеИмяФайла
			, СтрНайти(ВыбранноеИмяФайла, ".", НаправлениеПоиска.СКонца)+1)
		);
...
&НаКлиенте
Процедура ОбработчикОкончанияПомещенияЗаявкиТрейдГранит(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ОбработатьЗаявкуТрейдГранитНаСервере(Адрес
			, УникальныйИдентификатор
			, Сред(ВыбранноеИмяФайла
			, СтрНайти(ВыбранноеИмяФайла, ".", НаправлениеПоиска.СКонца)+1)
		);
...
&НаКлиенте
Процедура ОбработчикОкончанияПомещенияЗаявкиТоварСверху(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ОбработатьЗаявкуТоварСверхуНаСервере(Адрес
			, УникальныйИдентификатор
			, Сред(ВыбранноеИмяФайла
			, СтрНайти(ВыбранноеИмяФайла, ".", НаправлениеПоиска.СКонца)+1)
		);
	...
	Для Сч = 1 По ТабДок.ВысотаТаблицы Цикл
		...
		Если СтрНайти(ТекАртикул, "Артикул") Тогда
			...
			Для тСч = Сч + 1 По ТабДок.ВысотаТаблицы Цикл
				...
				НоваяСтрока.ДатаОтгрузки = ?(ЗначениеЗаполнено(НоваяСтрока.ПунктРазгрузки)
					, ерпсАктПриемкиПередачиСервер.ПолучитьДатуВывоза(Объект.Дата, НоваяСтрока.ПунктРазгрузки.ДеньВывоза)
					, Объект.Дата
				);
...
&НаСервере
Процедура ОбработатьЗаявкуХарьковскаяНаСервере(Адрес, УникальныйИдентификатор, РасширениеФайла)
	...
	Для Сч = 4 По ТабДок.ШиринаТаблицы Цикл
		ТекЗначение = ТабДок.Область(2, Сч, 2, Сч).Текст;
		Если Не ЗначениеЗаполнено(ТекЗначение) Тогда
			Продолжить;
		КонецЕсли;
		Если СтрНайти(ВРЕГ(ТекЗначение), "ПІДСУМОК") Тогда
			Прервать;
		КонецЕсли;
		
		СпрПунктРазгрузки = ерпсАктПриемкиПередачиСервер.ОпределитьПунктРазгрузки(
			СокрЛП(Лев(ТекЗначение, СтрНайти(ТекЗначение, " ")))
		);
		
		Если Не ЗначениеЗаполнено(СпрПунктРазгрузки) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон("Не встановлено відповідності або не знайдено пункт розвантаження %1", ТекЗначение)
			);
			Продолжить;
		КонецЕсли;
		
		МассивПунктовРазгрузки.Добавить(Новый Структура("Пункт,Колонка,ИДЗаявки", СпрПунктРазгрузки, Сч, СокрЛП(ТабДок.Область(3, Сч, 3, Сч).Текст)));		

	КонецЦикла;
	
	КолонкаНоменклатура = 3;
	
	Для Каждого Пункт Из МассивПунктовРазгрузки Цикл
		УИД = "ДопКол"+СтрЗаменить(Новый УникальныйИдентификатор(),"-","");
		Для Сч = 6 По ТабДок.ВысотаТаблицы Цикл
			ТекНоменклатура = СокрЛП(ТабДок.Область(Сч, КолонкаНоменклатура,Сч, КолонкаНоменклатура).Текст);			
			Если Не ЗначениеЗаполнено(ТекНоменклатура) Тогда
				Продолжить;
			КонецЕсли;
			
			Кво = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СокрЛП(ТабДок.Область(Сч, Пункт.Колонка,Сч, Пункт.Колонка).Текст));
			Если Не ЗначениеЗаполнено(Кво) тогда
				Продолжить;
			КонецЕсли;
			
			НоваяСтрока = Объект.Товары.Добавить();
			НоваяСтрока.ЗагруженнаяНоменклатура = СокрЛП(ТекНоменклатура);
			НоваяСтрока.ЗагруженноеКоличество = Кво;
            НоваяСтрока.ЗагруженныйДополнительныеАртикулы = СокрЛП(ТабДок.Область(Сч, КолонкаНоменклатура-1,Сч, КолонкаНоменклатура-1).Текст);
			
			НоваяСтрока.ДатаОтгрузки = ?(ЗначениеЗаполнено(Пункт.Пункт)
				, ерпсАктПриемкиПередачиСервер.ПолучитьДатуВывоза(Объект.Дата, Пункт.Пункт.ДеньВывоза)
				, Объект.Дата
			);
			
			НоваяСтрока.ИДЗаявкиКлиента = Пункт.ИДЗаявки;
			НоваяСтрока.Идентификатор = УИД;
			
			НоваяСтрока.ПунктРазгрузки = Пункт.Пункт;
			НоваяСтрока.ДополнительныеАртикулы = ерпсАктПриемкиПередачиСервер.ПолучитьДопАртикул(НоваяСтрока.ЗагруженныйДополнительныеАртикулы);
...
&НаСервере
Процедура ОбработатьЗаявкуТрейдГранитНаСервере(Адрес, УникальныйИдентификатор, РасширениеФайла)
	...
	МассивПунктовРазгрузки = Новый Массив;
	Для Сч = 3 По ТабДок.ШиринаТаблицы Цикл
		ТекЗначение = ТабДок.Область(3, Сч, 3, Сч).Текст;
		Если Не ЗначениеЗаполнено(ТекЗначение) Тогда
			Продолжить;
		КонецЕсли;
		Если СтрНайти(ВРЕГ(ТекЗначение), "ПІДСУМОК") Тогда
		...
		стрОТГ = СокрЛП(ТабДок.Область(2, Сч, 2, Сч).Текст);
		МассивОТГ = СтрРазделить(стрОТГ, ",");
		Если МассивОТГ.Количество() Тогда
			стрОТГ = СокрЛП(МассивОТГ[0]);
		Иначе
			стрОТГ = "";
		КонецЕсли;
		...
		МассивПунктовРазгрузки.Добавить(Новый Структура("Пункт,Колонка,ИДЗаявки", СпрПунктРазгрузки, Сч, СокрЛП(ТабДок.Область(6, Сч, 6, Сч).Текст)));		
	...
	Для Каждого Пункт Из МассивПунктовРазгрузки Цикл
		УИД = "ДопКол"+СтрЗаменить(Новый УникальныйИдентификатор(),"-","");
		Для Сч = 11 По ТабДок.ВысотаТаблицы Цикл
			ТекНоменклатура = СокрЛП(ТабДок.Область(Сч, КолонкаНоменклатура,Сч, КолонкаНоменклатура).Текст);			
			Если Не ЗначениеЗаполнено(ТекНоменклатура) Тогда
				Продолжить;
			КонецЕсли;
			Если СокрЛП(ТекНоменклатура) = "Разом" Тогда
				Продолжить;
			КонецЕсли;
			
			ТекАртикул = Лев(СокрЛП(ТекНоменклатура), 5);
			ТекНоменклатура = Сред(СокрЛП(ТекНоменклатура), 6, СтрДлина(СокрЛП(ТекНоменклатура)));
			...
			НоваяСтрока.ЗагруженнаяНоменклатура = СокрЛП(ТекНоменклатура);
			НоваяСтрока.ЗагруженноеКоличество = Кво;
            НоваяСтрока.ЗагруженныйДополнительныеАртикулы = ТекАртикул;
...
&НаСервере
Процедура ОбработатьЗаявкуТоварСверхуНаСервере(Адрес, УникальныйИдентификатор, РасширениеФайла)
	...
	ПерваяКолонка = 2;
	Для Сч = 1 По ТабДок.ВысотаТаблицы Цикл
		Если ВРег(СокрЛП(ТабДок.Область(Сч, ПерваяКолонка,Сч, ПерваяКолонка).Текст)) = ВРег("ЗаказПокупателя.Період") Тогда
			ПерваяСтрока = Сч;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Для Сч = ПерваяСтрока + 4 По ТабДок.ВысотаТаблицы Цикл
		СтрИдЗаказа = ТабДок.Область(Сч, ПерваяКолонка, Сч, ПерваяКолонка).Текст;
		Если СокрЛП(СтрИдЗаказа) = "Підсумок" Тогда
			Прервать;
		КонецЕсли;
		МассивИДЗаказа = СтрРазделить(СтрИдЗаказа, " ");
		ИДЗаказа = ?(МассивИДЗаказа.ВГраница() > 3, МассивИДЗаказа[2], "");
		
		СтрПунктРазгрузки = ТабДок.Область(Сч, ПерваяКолонка+1, Сч, ПерваяКолонка+1).Текст;
		СпрПунктРазгрузки = ерпсАктПриемкиПередачиСервер.ОпределитьПунктРазгрузки(СокрЛП(СтрПунктРазгрузки));
		
		УИД = "ДопКол"+СтрЗаменить(Новый УникальныйИдентификатор(),"-","");
		
		Для Кч = ПерваяКолонка + 2 По ТабДок.ШиринаТаблицы Цикл
			СтрТовар = СокрЛП(ТабДок.Область(ПерваяСтрока, Кч, ПерваяСтрока, Кч).Текст);
			Если СокрЛП(СтрТовар) = "Підсумок" Тогда
				Прервать;
			КонецЕсли;
			
			Кво = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(
				СтрЗаменить(СокрЛП(ТабДок.Область(Сч, Кч, Сч, Кч).Текст), ",", ".")
			);
			
			Если Не ЗначениеЗаполнено(Кво) Тогда
				Продолжить;
			КонецЕсли;
			СтрАртикул = СокрЛП(Лев(СтрТовар, СтрНайти(СтрТовар, " ", НаправлениеПоиска.СНачала)));
			
			НоваяСтрока = Объект.Товары.Добавить();
			НоваяСтрока.ЗагруженнаяНоменклатура = СокрЛП(СтрЗаменить(СтрЗаменить(СтрТовар, ", кг", ""), СтрАртикул, ""));
			НоваяСтрока.ЗагруженноеКоличество =  Кво;
			НоваяСтрока.ЗагруженныйДополнительныеАртикулы = СтрАртикул;
			
			НоваяСтрока.ДатаОтгрузки = ?(ЗначениеЗаполнено(СпрПунктРазгрузки)
				, ерпсАктПриемкиПередачиСервер.ПолучитьДатуВывоза(Объект.Дата, СпрПунктРазгрузки.ДеньВывоза)
				, Объект.Дата
			);
			Если Не ЗначениеЗаполнено(НоваяСтрока.ДатаОтгрузки) Тогда
				НоваяСтрока.ДатаОтгрузки = Объект.Дата;
			КонецЕсли;
			НоваяСтрока.ИДЗаявкиКлиента = ИДЗаказа;
			НоваяСтрока.Идентификатор = УИД;
			
			НоваяСтрока.ПунктРазгрузки = СпрПунктРазгрузки;
			НоваяСтрока.ДополнительныеАртикулы = ерпсАктПриемкиПередачиСервер.ПолучитьДопАртикул(НоваяСтрока.ЗагруженныйДополнительныеАртикулы);
			НоваяСтрока.Количество = Кво;
...
&НаКлиенте
Процедура ЗагрузитьСканыРеализации(Команда)
...
&НаКлиенте
Процедура ЗагрузитьСканыТТН(Команда)
...
Процедура ВыборКаталогаЗавершение(Результат, ДопПараметры) Экспорт
	...
	НачатьПоискФайлов(Новый ОписаниеОповещения("ПоискФайловЗавершение", ЭтотОбъект, ДопПараметры), Путь, "*.*", Ложь);
...
Процедура ПоискФайловЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Файл Из НайденныеФайлы Цикл
		...
		ПрикрепитьКДокументуНаСервере(ДД
...
&НаСервере
Процедура ПрикрепитьКДокументуНаСервере(ДД, Знач ИмяБезРасширения, Знач РасширениеБезТочки, МассивЗагруженных, МассивНеЗагруженных, Префикс = "")
	...
	ИмяБезРасширенияДляПоиска = СтрЗаменить(ИмяБезРасширенияДляПоиска, Префикс, "");
...
Процедура ОткрытьПрисоединенныеФайлы(Команда)
	...
	Если ЗначениеЗаполнено(ТекДанные.Реализация) Тогда
		РаботаСФайламиКлиент.ОткрытьФормуВыбораФайлов(ТекДанные.Реализация, ЭтотОбъект,);
	КонецЕсли;
...
&НаКлиенте
Процедура КомплектДокументовДляПечатиПриИзменении(Элемент)
	СохранитьКомплектПечатиНаСервере();
КонецПроцедуры

&НаСервере
Процедура СохранитьКомплектПечатиНаСервере()
	УстановитьПривилегированныйРежим(Истина);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("КомплектДокументовДляПечати", "КомплектДокументовДляПечати", КомплектДокументовДляПечати.Выгрузить(),, Пользователи.ТекущийПользователь());
	УстановитьПривилегированныйРежим(Ложь);
КонецПроцедуры
...
Процедура ОчиститьНастройки(Команда)
	...
	ОчиститьНастройкиКомплектаПечатиНаСервере();

КонецПроцедуры

&НаСервере
Процедура ОчиститьНастройкиКомплектаПечатиНаСервере()
	УстановитьПривилегированныйРежим(Истина);
	КомплектДокументовДляПечати.Очистить();
	ОбщегоНазначения.ХранилищеНастроекДанныхФормУдалить("КомплектДокументовДляПечати", "КомплектДокументовДляПечати",Пользователи.ТекущийПользователь());
	УстановитьПривилегированныйРежим(Ложь);
КонецПроцедуры
...
Процедура ПечатьКомплектаДокументов(Команда)
	...
	Если Модифицированность Тогда
		ПоказатьПредупреждение(, "Треба записати документ");
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаПечатьКОмплектаДокументов", ЭтотОбъект)
		, "Друкувати комплект документів водію?"
		, РежимДиалогаВопрос.ДаНет
	);
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияВопросаПечатьКОмплектаДокументов(Результат, ДопПараметры) Экспорт
	...
	//ПечатьКомплектаНаСервере(
	//	?(ЗначениеЗаполнено(ТекДанные.ДатаВывозаФакт), ТекДанные.ДатаВывозаФакт, ТекДанные.ДатаОтгрузки)
	//	, ?(ЗначениеЗаполнено(ТекДанные.ВодительФакт), ТекДанные.ВодительФакт, ТекДанные.Водитель)
	//	, ?(ЗначениеЗаполнено(ТекДанные.АвтоФакт), ТекДанные.АвтоФакт, ТекДанные.Авто)
	//);
	
	//Для Каждого ТекДанные Из Объект.ПунктыРазгрузки.НайтиСтроки(Новый Структура()) Цикл
	
	тзПунктыРазгрузки = ПолучитьПунктыРазгрузкиПоВодителю(ТекДанные.Водитель, ТекДанные.Авто, ТекДанные.ДатаОтгрузки);
	
	Для Каждого Эл Из тзПунктыРазгрузки Цикл
	
		Для Каждого Док Из КомплектДокументовДляПечати Цикл
			Если Док.Документ = "Реалізація" ИЛИ Док.Документ = "Реализация" Тогда
				Для Сч = 1 По Док.Количество Цикл
					ТабДок = ПечатьРННаСервере(ТекДанные.Водитель
						, ТекДанные.Авто
						, Эл
						, ТекДанные.ДатаОтгрузки
					);
					//ТабДок.Показать("РН_"+СокрЛП(ТекДанные.Водитель)+"_"+СокрЛП(ТекДанные.Авто));
					ТабДок.Напечатать(РежимИспользованияДиалогаПечати.НеИспользовать);
				КонецЦикла;
			
			ИначеЕсли Док.Документ = "ТТН" Тогда
				Для Сч = 1 По Док.Количество Цикл
					ТабДок = ПечатьТТТННаСервере(ТекДанные.Водитель
						, ТекДанные.Авто
						, Эл
						, ТекДанные.ДатаОтгрузки
						, Истина
					);
					//ТабДок.Показать("ТТН_"+СокрЛП(ТекДанные.Водитель)+"_"+СокрЛП(ТекДанные.Авто));
					ТабДок.Напечатать(РежимИспользованияДиалогаПечати.НеИспользовать);
				КонецЦикла;
				
			ИначеЕсли Док.Документ = "Декларация" ИЛИ Док.Документ = "Декларація" Тогда
				Для Сч = 1 По Док.Количество Цикл
					ТабДок = ПечатьДекларацииНаСервере(ТекДанные.Водитель
						, ТекДанные.Авто
						, Эл
						, ТекДанные.ДатаОтгрузки
					);
					//ТабДок.Показать("Декларация_"+СокрЛП(ТекДанные.Водитель)+"_"+СокрЛП(ТекДанные.Авто));
					ТабДок.Напечатать(РежимИспользованияДиалогаПечати.НеИспользовать);
				КонецЦикла;
...
&НаСервере
Функция ПолучитьПунктыРазгрузкиПоВодителю(Водитель, Авто, ДатаВывоза)
	...
	Запрос.УстановитьПараметр("ДатаВывоза", ДатаВывоза);
	Запрос.УстановитьПараметр("Таб", Объект.ПунктыРазгрузки.Выгрузить());
	Запрос.Текст = "ВЫБРАТЬ Т.* ПОМЕСТИТЬ ВТТаб ИЗ &Таб КАК Т;
	|ВЫБРАТЬ Т.ПунктРазгрузки, Т.Идентификатор
	|ИЗ ВТТаб КАК Т 
	|ГДЕ ВЫБОР КОГДА Т.ВодительФакт = Значение(Справочник.Сотрудники.ПустаяСсылка) ТОГДА Т.Водитель ИНАЧЕ Т.ВодительФакт КОНЕЦ = &Водитель
	|	И ВЫБОР КОГДА Т.АвтоФакт = Значение(Справочник.Авто.ПустаяСсылка) ТОГДА Т.Авто ИНАЧЕ Т.АвтоФакт КОНЕЦ = &Авто
	|	И ВЫБОР КОГДА Т.ДатаВывозаФакт = ДАТАВРЕМЯ(1,1,1) ТОГДА Т.ДатаОтгрузки ИНАЧЕ Т.ДатаВывозаФакт КОНЕЦ = &ДатаВывоза";

	Т = Запрос.Выполнить().Выгрузить();
	
	Для Каждого сТ Из Т Цикл
		МассивСтрок = Объект.ПунктыРазгрузки.НайтиСтроки(Новый Структура("ПунктРазгрузки,Идентификатор",сТ.ПунктРазгрузки, сТ.Идентификатор));
		Для Каждого Эл Из МассивСтрок Цикл
		
		
			Если СоздатьРТУНаСервере(Эл.ПунктРазгрузки, Эл.Идентификатор, Ложь) Тогда
				Эл.СтадияГотовыеДокументы = Истина;
			КонецЕсли; 
		КонецЦикла;
	КонецЦикла;
	
	Т.Свернуть("ПунктРазгрузки");
	
	Возврат Т.ВыгрузитьКолонку("ПунктРазгрузки");
