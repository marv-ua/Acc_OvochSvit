...
Процедура СоздатьАктыНаСервереНовый(Выб)
	...
	Для каждого Пункт из ОБъект.ПунктыРазгрузки Цикл
		...
		Если АктУжеСозданРаннее(Пункт.Контрагент,Пункт.ПунктРазгрузки,Объект.Ссылка,, Пункт.Идентификатор) тогда
			Продолжить;
		КонецЕсли;
		...
		НовДок.Идентификатор	= Пункт.Идентификатор; 
		//ек++
		...
		СтрокиКПереносу = Объект.Товары.НайтиСтроки(Новый Структура("ПунктРазгрузки,Идентификатор",Пункт.ПунктРазгрузки, Пункт.Идентификатор));
...
Процедура СоздатьЗаказНаСборкуНаСервере(Идентификатор, ПунктРазгрузки, ДатаОтгрузки, ЗаказНаЗборку = Неопределено)
	...
	ЗаказНаЗборку = ЕстьЗаказнаСборку(Объект.Ссылка, Идентификатор);
...
&НаСервереБезКонтекста
Функция ЕстьЗаказнаСборку(Заявка, Идентификатор)
