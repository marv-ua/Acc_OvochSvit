...
Процедура ПунктыРазгрузкиПунктРазгрузкиПриИзменении(Элемент)
	...
	ТЧТоварыИзменитьПунктОтгрузки(ТекДанные.ПунктРазгрузки, ТекДанные.ДатаОтгрузки);
...
&НаСервере
Функция СоздатьЗаявкуНаСервере(ТекСтрока)
	ТекДанные = Объект.ПунктыРазгрузки.НайтиПоИдентификатору(ТекСтрока);
	...
	ЗаполнитьЗначенияСвойств(Об, Объект,, "Ответственный,Дата,Номер,Комментарий,Регион,ДокументОснования");
	Об.Дата = ТекДанные.ДатаОтгрузки;
...
Процедура СоздатьЗаявкиНаСервере(Успешно)
	...
	// Test}
	тзРодитель.Свернуть("ДатаОтгрузки, ПунктРазгрузки, Регион");
		...
		Для Каждого Стр Из тзДатаПунктРегионСклад.НайтиСтроки(Новый Структура("ДатаОтгрузки,Регион", СтрокаРодитель.ДатаОтгрузки, СтрокаРодитель.Регион)) Цикл
			Т0 = тзТовары.Скопировать(
				тзТовары.НайтиСтроки(
					Новый Структура("ДатаОтгрузки,ПунктРазгрузки", СтрокаРодитель.ДатаОтгрузки, Стр.ПунктРазгрузки)
				)
			);
			...
			Для Каждого С Из Т1 Цикл
				МассивСтрок = Об.ПунктыРазгрузки.НайтиСтроки(Новый Структура("ДатаОтгрузки,ПунктРазгрузки", СтрокаРодитель.ДатаОтгрузки, Стр.ПунктРазгрузки));
				Если МассивСтрок.Количество() Тогда
					Продолжить;
				КонецЕсли;
...
&НаСервере
Процедура ТЧТоварыИзменитьПунктОтгрузки(ПунктРазгрузки, Дата)
