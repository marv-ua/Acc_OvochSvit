...
Процедура Обработано(Команда)
	...
	Если М.Количество() Тогда
		ПоказатьПредупреждение(,
			"Замовлення на збирання які вже зібрані було оновлено, треба попередити комірників" + Символы.ПС +Символы.ПС + СтрСоединить(М, Символы.ПС),
			,
			"Замовлення на збирання оновлено"
		);
	КонецЕсли;
