...
Процедура СоздатьЗаявкиНаСервере(Успешно)
	...
	тзРодитель.Свернуть("ДатаОтгрузки,ДатаВывозаФакт,Регион");
...
Процедура СоздатьАктыНаСервереНовый(Выб)
	...
	Для каждого Пункт из ОБъект.ПунктыРазгрузки Цикл
		...
		НовДок.Перевозчик		= ?(ЗначениеЗаполнено(Пункт.ВодительФакт), Пункт.ВодительФакт, Пункт.Водитель);
		НовДок.Перевозчик		= ?(ЗначениеЗаполнено(Пункт.АвтоФакт), Пункт.АвтоФакт, Пункт.Авто);
		НовДок.Перевозчик		= ?(ЗначениеЗаполнено(Пункт.ПрицепФакт), Пункт.ПрицепФакт, Пункт.Прицеп);
