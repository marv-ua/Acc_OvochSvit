...
Функция ПечатьМ21(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	|	Документ.ИнвентаризацияТоваровНаСкладе КАК ИнвентаризацияТоваровНаСкладе
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ОтветственныеЛица.СрезПоследних(&ДатаДок, СтруктурнаяЕдиница = &СтруктурнаяЕдиница) КАК ОтветственныеЛица
	|	ПО
	|		ОтветственныеЛица.СтруктурнаяЕдиница = ИнвентаризацияТоваровНаСкладе.Склад
	|
	|ГДЕ
	|	ИнвентаризацияТоваровНаСкладе.Ссылка = &ТекущийДокумент
	|";
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		ЗапросДокумент.УстановитьПараметр("СтруктурнаяЕдиница", Ссылка.Склад);
		ЗапросДокумент.УстановитьПараметр("ДатаДок", Ссылка.Дата);
