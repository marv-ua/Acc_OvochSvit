...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПередачаМалоценныхАктивовВЭксплуатацию") Тогда
		Дата				= Основание.Дата;
		Организация			= Основание.Организация;
		ДокументОснование	= Основание;
		ПодразделениеОрганизации = Основание.ПодразделениеОрганизации;
		...
		Для Каждого СтрОснования Из Основание.МалоценныеАктивы Цикл
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |				И ФизическоеЛицо = &ФизическоеЛицо) КАК КадроваяИсторияСотрудниковСрезПоследних";
			Запрос.УстановитьПараметр("Период", Дата);
			Запрос.УстановитьПараметр("ФизическоеЛицо", СтрОснования.ФизЛицо);
			Запрос.УстановитьПараметр("Организация", Организация);
			Выборка = Запрос.Выполнить().Выбрать();
			
			Если Выборка.Следующий() Тогда
				НовСтр.МОЛДолжность = СокрЛП(ПолучитьТекущуюДолжностьСотрудника(Выборка.Сотрудник, Дата)); 
			КонецЕсли;			
		КонецЦикла;		
