...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//МестоСоставленияДокумента = "";
	//ПредставительОрганизации = Справочники.ФизическиеЛица.ПустаяСсылка();
	Получил  =  "";
	...
	ерпсАктПриемкиПередачи = Неопределено;
	а_Периоды = Справочники.а_Периоды.ПустаяСсылка();
	Идентификатор = ""; 
	ИДЗаявкиКлиента = ""; 
	ДатаЗаявкиКлиента = Неопределено;
	// Test}
	//...ЕРПС.Кононенко
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//ерпс
	ерпсВес = Товары.Итог("Количество");
	//...ерпс
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ERPS_TASK В.Головченко_19.01.2023
	УстановитьПривилегированныйРежим(Истина);
	Если ТипЗнч(Сделка) = Тип("ДокументСсылка.ерпсАктПриемкиПередачиТоваров")
		или ерпсАктПриемкиПередачи <> Документы.ерпсАктПриемкиПередачиТоваров.ПустаяСсылка() тогда
		Если ерпсАктПриемкиПередачиСервер.ТабличныеЧастыСоответствуютПолностью(Ссылка,?(ерпсАктПриемкиПередачи <> Документы.ерпсАктПриемкиПередачиТоваров.ПустаяСсылка(),ерпсАктПриемкиПередачи,Сделка)) тогда
			ерпсАктПриемкиПередачиСервер.ОбновитьСтатусАктаПриемкиПередачиТоваров(?(ерпсАктПриемкиПередачи <> Документы.ерпсАктПриемкиПередачиТоваров.ПустаяСсылка(),ерпсАктПриемкиПередачи,Сделка), "Отгружено");
		КонецЕсли;                                                                     
	КонецЕсли;
...
//ERPS_TASK 9048_В.Головченко_28.11.2022
Процедура ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)
	Если ерпсАктПриемкиПередачи <> Документы.ерпсАктПриемкиПередачиТоваров.ПустаяСсылка() тогда
		СтандартнаяОбработка = ЛОжь;
		РегламентированнаяОтчетность.ДобавитьПрефиксОрганизации(ЭтотОбъект, Префикс); 
		Номер = Префикс + Сред(ерпсАктПриемкиПередачи.Номер,5,СтрДлина(ерпсАктПриемкиПередачи.Номер)) +"/"+ КоличествоДокументовРТУНаОснованииАктаПриемаПередачиТовара(ерпсАктПриемкиПередачи);
	Иначе
        СтандартнаяОбработка = Ложь;
		...
		Пока СтрДлина(Префикс4) Цикл

		        Префикс4 = Префикс4 + "0";
		КонецЦикла;
			
		Если ПустаяСтрока(Префикс4) Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОДСТРОКА(Документ.Номер, 1, 4) = &Префикс", "Истина");
		КонецЕсли;				
		...
		Если Выборка.Следующий() Тогда
		    Попытка
		     ...
		     Номер = Префикс+Формат(НомерЧисло,"ЧЦ="+Строка(СтрДлина(Номер)-СтрДлина(Префикс))+"; ЧВН=; ЧГ=");    
		    Исключение
		    КонецПопытки;
		Иначе    
		     НомерЧисло = 1;
...
Функция КоличествоДокументовРТУНаОснованииАктаПриемаПередачиТовара(Сделка)
	...
	Запрос.Текст = 
		...
		|	И РеализацияТоваровУслуг.ерпсАктПриемкиПередачи = &ерпсАктПриемкиПередачи";
	
	Запрос.УстановитьПараметр("ерпсАктПриемкиПередачи", Сделка);
