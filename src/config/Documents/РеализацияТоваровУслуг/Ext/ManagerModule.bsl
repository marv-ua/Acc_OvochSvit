...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "Акт отгрузки";
	КомандаПечати.Представление = НСтр("ru='Акт отгрузки';uk='Відвантажувальний акт'");
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ВыполнитьКомандуПечати";
	КомандаПечати.СписокФорм    = "ФормаСписка,ФормаВыбора,ФормаДокументаОбщая";
	...
	// Товарно-транспортная накладная (1-Т) упрощенная без цен
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.ПечатьТТН";
	КомандаПечати.Идентификатор = "ТТН_Упрощенная_БезЦен";
	КомандаПечати.Представление = НСтр("ru='Товарно-транспортная накладная (1-ТН) дод.';uk='Товарно-транспортна накладна (1-ТН) дод.'");
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ОткрытьФормуПечатиТТН";
	КомандаПечати.СписокФорм    = "ФормаСписка,ФормаВыбора,ФормаДокументаОбщая";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Акт отгрузки") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Акт отгрузки", НСтр("ru='Акт отгрузки';uk='Відвантажувальний акт'"), 
			ПечатьАктаОтгрузки(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,"Документ.РеализацияТоваровУслуг.ПФ_MXL_Накладная3", , Истина);
	КонецЕсли;
	//...ерпс
...
//	|	Ответственный.ФизЛицо.Наименование КАК Отпустил,
	...
	|	,ИДЗаявкиКлиента КАК ИДЗаявкиКлиента
	|	,ДатаЗаявкиКлиента КАК ДатаЗаявкиКлиента
	//ек
	|ИЗ
...
//
	...
	//	КонецЕсли;
		ОбластьМакета.Параметры.Период = СтрШаблон("%1	ID заявки: %2"
			, ОбластьМакета.Параметры.Период
			, Шапка.ИДЗаявкиКлиента
		);
		ТабДокумент.Вывести(ОбластьМакета);
		//ек
...
// Test{20240410 
Функция ПечатьАктаОтгрузки(МассивОбъектов, ОбъектыПечати, ПараметрыВывода) Экспорт
...
//	|	Ответственный.ФизЛицо.Наименование КАК Отпустил,
	...
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.АвтоМасштаб = Истина;
	ТабДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РеализацияТоваровУслуг_Накладная3";

	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.ПФ_MXL_Накладная3");
	...
	Для каждого Ссылка Из МассивОбъектов Цикл	
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Поставщик");
		ОбластьМакета.Параметры.Заполнить(Шапка);
		ОбластьМакета.Параметры.ПредставлениеПоставщика = СведенияОПоставщике.Представление;
		ОбластьМакета.Параметры.РеквизитыПоставщика =     ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПоставщике, "КодПоЕДРПОУ,КодПоДРФО,ИНН,",,КодЯзыкаПечать);
	    ТабДокумент.Вывести(ОбластьМакета);
		
		
		ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
		ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Акт отгрузки';uk='Відвантажувальний акт'",КодЯзыкаПечать),КодЯзыкаПечать);
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Покупатель");
		ОбластьМакета.Параметры.Заполнить(Шапка);
	 	ОбластьМакета.Параметры.ПредставлениеПокупателя = СведенияОПокупателе.Представление;//ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование,",,КодЯзыкаПечать);
		ОбластьМакета.Параметры.РеквизитыПокупателя		= ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПокупателе,"КодПоЕДРПОУ,КодПоДРФО,ИНН,",,КодЯзыкаПечать);
		ТабДокумент.Вывести(ОбластьМакета);
		...
		Область = Макет.ПолучитьОбласть("Получатель");
		Область.Параметры.Получатель = "Одержувач: " +
			Шапка.АдресДоставки;
		ТабДокумент.Вывести(Область);
		
		Область = Макет.ПолучитьОбласть("ПолучательДоп");
		Область.Параметры.ПолучательДоп = "" + "Період " + СокрЛП(СтрШаблон("%1 (%2)", Шапка.Период.ПредставлениеПериода, Шапка.Период)) + "; " +"id " + Шапка.ИДЗаявкиКлиента + "; " + Шапка.Получил;
		ТабДокумент.Вывести(Область);
