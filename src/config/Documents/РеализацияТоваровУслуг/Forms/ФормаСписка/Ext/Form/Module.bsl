...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ВозвратРН" Или Поле.Имя = "ОригиналТТН" Или Поле.Имя = "Погоджено" Тогда
		СтандартнаяОбработка = Ложь;
		//ЕРПС.Кононенко
		Если СписокВыборПроверка() тогда
			...
			КонецЕсли;
		ИначеЕсли Поле.Имя <> "Погоджено" Тогда
			рез = СписокВыбор_ИзменитьРеквизит(ВыбраннаяСтрока,Поле.Имя);
			Если ТипЗнч(рез) = Тип("Булево") Тогда
				Элемент.ТекущиеДанные[Поле.Имя] = рез;
				Элементы.Список.Обновить();
...
Функция СписокВыбор_ИзменитьРеквизит(Ссылка,Имя)
	Если Не ЗначениеЗаполнено(Ссылка) Или (Имя <> "ВозвратРН" И Имя <> "ОригиналТТН" И Имя <> "Погоджено") Тогда	
	...
	Попытка
		...
		Если (СокрЛП(Пользователи.ТекущийПользователь()) = "DNIPRO10" ИЛИ СокрЛП(Пользователи.ТекущийПользователь()) = "DNIPRO2")
			И (Имя = "ОригиналТТН" ИЛИ Имя = "ВозвратРН") Тогда
