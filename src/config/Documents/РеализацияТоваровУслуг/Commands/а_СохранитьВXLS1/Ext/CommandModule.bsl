...
Функция ПолучитьТаблицуНаСервере(ПараметрКоманды, ИмяФайла)
	...
	Для Каждого Док Из ПараметрКоманды Цикл 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	РеализацияТоваровУслуг.ИДЗаявкиКлиента КАК ИДЗаявкиКлиента
		               |ИЗ
...
//				Область.Параметры.СуммаБезНДС = Формат(Выборка.Сумма, "ЧДЦ=2; ЧГ=");
			...
			Область.Параметры.Водитель = СтрШаблон("%1, %2",Выборка.Водитель, Выборка.Водитель.Права);
			
			// Администратор{20240813 
			Область.Параметры.ПунктРазгрузки = СтрШаблон("%2, %1", Выборка.ПунктРазгрузки, Выборка.ИДЗаявкиКлиента);
			// Администратор}
