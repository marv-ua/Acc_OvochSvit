...
&НаКлиенте
Процедура ТоварыДопАртикулПриИзменении(Элемент)
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
    ...
    Масс = Объект.ДопАртикулыТоваров.НайтиСтроки(Отбор);
    Для каждого Строка из Масс Цикл
        Объект.ДопАртикулыТоваров.Удалить(Строка);
	КонецЦикла;
	
	Для Каждого Эл Из Данные.ДопАртикул Цикл
		НоваяСтрока = Объект.ДопАртикулыТоваров.Добавить();
		НоваяСтрока.Идентификатор = Данные.Идентификатор;
		НоваяСтрока.ДополнительныеАртикулы = Эл.Значение;
	КонецЦикла;
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Элемент.ТекущиеДанные.ДопАртикул.ТипЗначения =
		Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ДополнительныеАртикулы"))
	);
...
&НаКлиенте
Процедура ДопАртикулыТоваровПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)

	Данные = Элементы.Товары.ТекущиеДанные;
    ТекДанные = Элементы.ДопАртикулыТоваров.ТекущиеДанные;
    Если НоваяСтрока Тогда
        ТекДанные.Идентификатор = Данные.Идентификатор;
	КонецЕсли;
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	Элементы.ДопАртикулыТоваров.ОтборСтрок = Новый ФиксированнаяСтруктура(Отбор);
...
&НаСервере
Процедура ОбновитьКолонкуДопАртикулы()
	
	Для Каждого Товар Из Объект.Товары Цикл
		
		Товар.ДопАртикул.Очистить();
		
		МассивСтрок = Объект.ДопАртикулыТоваров.НайтиСтроки(Новый Структура("Идентификатор", Товар.Идентификатор));
		Для Каждого Артикул Из МассивСтрок Цикл
			
			Товар.ДопАртикул.Добавить(Артикул.ДополнительныеАртикулы);
