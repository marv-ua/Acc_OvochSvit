...
Процедура РасшифровкаПлатежаСделкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.ДокументРасчетовСКонтрагентом.ФормаВыбора", ПараметрыФормы, Элемент,,,, Новый ОписаниеОповещения("ВыборРасшифровкиПлатежаЗавершение", ЭтотОбъект));
	// Test}
КонецПроцедуры

// Test{20240131
&НаКлиенте
Процедура ВыборРасшифровкиПлатежаЗавершение(Результат, ДопПараметры) Экспорт
	...
	Для Каждого Элемент Из Результат Цикл
		...
		НоваяСтрока.ДоговорКонтрагента = ЗначениеРеквизита(Элемент, "ДоговорКонтрагента");
		НоваяСтрока.СуммаПлатежа = ЗначениеРеквизита(Элемент, "СуммаДокумента");
		НоваяСтрока.Сделка = Элемент; 
		
		
		СтрокаПлатеж = НоваяСтрока;
		...
		ЗаполнитьЗначенияСвойств(СтрокаПлатеж, СвойстваПлатежа,, "Сделка");
