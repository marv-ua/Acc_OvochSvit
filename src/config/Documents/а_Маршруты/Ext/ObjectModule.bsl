...
Процедура ЗаполнитьМаршруты() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И Заявка.Ссылка.Дата МЕЖДУ &Дата1 И КОНЕЦПЕРИОДА(&Дата2, ДЕНЬ)";
	...
	Если Не ЗначениеЗаполнено(ПериодНачало) И Не ЗначениеЗаполнено(ПериодОкончание) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Заявка.Ссылка.Дата МЕЖДУ &Дата1 И КОНЕЦПЕРИОДА(&Дата2, ДЕНЬ)", "");
	КонецЕсли;
