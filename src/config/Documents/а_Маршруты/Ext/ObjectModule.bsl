...
Процедура ЗаполнитьМаршруты() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И Заявка.ДатаОтгрузки МЕЖДУ &Дата1 И КОНЕЦПЕРИОДА(&Дата2, ДЕНЬ)";
	...
	Если Не ЗначениеЗаполнено(ПериодНачало) И Не ЗначениеЗаполнено(ПериодОкончание) Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Заявка.ДатаОтгрузки МЕЖДУ &Дата1 И КОНЕЦПЕРИОДА(&Дата2, ДЕНЬ)", "");
	КонецЕсли;
