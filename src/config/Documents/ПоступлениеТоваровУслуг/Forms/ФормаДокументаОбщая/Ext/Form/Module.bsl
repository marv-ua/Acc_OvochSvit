...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ерпсНикодаЕвгений 2021-10-12
	//Установка доступности реквизита СвереноПН (сверка с Налоговой накладной) 
	ЭтаФорма.Элементы.СвереноПН.Доступность = ПроверкаСвереноПН();
	
	//Установка доступности изменения Данных для Товаров и Услуг в таблице  
	Если Объект.СвереноПН Тогда
		Если ПроверкаСвереноПН()Тогда
			Элементы.Товары.Доступность	= Объект.СвереноПН;
		    Элементы.Услуги.Доступность	= Объект.СвереноПН;	
		Иначе
			Элементы.Товары.Доступность	= Не(Объект.СвереноПН);
		    Элементы.Услуги.Доступность	= Не(Объект.СвереноПН);
		КонецЕсли;
	КонецЕсли;
	//...ерпсНикодаЕвгений 2021-10-12
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ерпсЗаполнитьСписокВыбораПунктПогрузки();
	КонецЕсли;
	//...#9148
	
	//Элементы.СчетПДФО.СписокВыбора.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("6411А"));
	//Элементы.СчетПДФО.СписокВыбора.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("6411 ГПХ"));
	//
	//Элементы.СубконтоВоенныеСбор1.СписокВыбора.Добавить(Справочники.Налоги.НайтиПоКоду("ВоенныйСборДЦПХ"));
	//Элементы.СубконтоВоенныеСбор1.СписокВыбора.Добавить(Справочники.Налоги.НайтиПоНаименованию("Військовий збір з оренди"));
...
////////////////////////////////////////////////////////////////////////////////
// ЕРПС СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ

&НаСервере
Функция ПроверкаСвереноПН()
	//ерпсНикодаЕвгений 2021-10-11
	Если РольДоступна(Метаданные.Роли.ЕРПССогласованиеРеализации) Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции //ПроверкаСвереноПН()...ерпсНикодаЕвгений 2021-10-11

&НаСервереБезКонтекста
Функция ерпсПолучитьПунктыПогрузки(Контрагент)
	//#8950
	Результат = Новый Структура;
	
	Результат.Вставить("ЮридическийАдрес",	БухгалтерскийУчетПереопределяемый.ПолучитьАдресИзКонтактнойИнформации(Контрагент, "Юридический"));
	Результат.Вставить("ФактическийАдрес",	БухгалтерскийУчетПереопределяемый.ПолучитьАдресИзКонтактнойИнформации(Контрагент, "Фактический"));
	
	Возврат Результат;
КонецФункции

&НаСервере
Процедура ерпсЗаполнитьСписокВыбораПунктПогрузки()
	
	//#8950
	ПунктыПогрузки = ерпсПолучитьПунктыПогрузки(Объект.Контрагент);
	
	Элементы.ерпсПунктПогрузки.СписокВыбора.Очистить();
	
	Если ЗначениеЗаполнено(ПунктыПогрузки.ЮридическийАдрес) Тогда 
		Элементы.ерпсПунктПогрузки.СписокВыбора.Добавить(ПунктыПогрузки.ЮридическийАдрес);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПунктыПогрузки.ФактическийАдрес) Тогда 
		Элементы.ерпсПунктПогрузки.СписокВыбора.Добавить(ПунктыПогрузки.ФактическийАдрес);
	КонецЕсли;
	
	// Test{20230922
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	АдресаДоставки.Наименование КАК Наименование,
	               |	АдресаДоставки.Представление КАК Представление
	               |ИЗ
	               |	Справочник.АдресаДоставки КАК АдресаДоставки
	               |ГДЕ
	               |	НЕ АдресаДоставки.ПометкаУдаления
	               |	И АдресаДоставки.Владелец = &Владелец";
	Запрос.УстановитьПараметр("Владелец", Объект.Контрагент);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.Представление) Тогда 
			Элементы.ерпсПунктПогрузки.СписокВыбора.Добавить(Выборка.Представление);
		ИначеЕсли ЗначениеЗаполнено(Выборка.Наименование) Тогда 
			Элементы.ерпсПунктПогрузки.СписокВыбора.Добавить(Выборка.Наименование);	
		КонецЕсли;
	КонецЦикла;
	// Test}
...
&НаКлиенте
Процедура СчетПДФОПриИзменении(Элемент)
	Объект.СубконтоПДФО = Объект.Контрагент;
КонецПроцедуры
