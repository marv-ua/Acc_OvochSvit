...
Функция ПечатьПоступлениеТоваров(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл	
		...
		Для каждого ВыборкаСтрокТовары Из ЗапросТовары Цикл 
			...
			//ерпс
			Если ЗначениеЗаполнено(ВыборкаСтрокТовары.Номенклатура.НоменклатураГТД) тогда
				ОбластьМакета.Параметры.НП = ВыборкаСтрокТовары.Номенклатура.НоменклатураГТД;
			КонецЕсли;
			Если ВыборкаСтрокТовары.Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.НайтиПоКоду("000000005") тогда
				ОбластьМакета.Параметры.Импорт = "Х";
			Иначе
				ОбластьМакета.Параметры.Импорт = "";	
			КонецЕсли;
			//...ерпс
