...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Контрагенты.Ссылка В ИЕРАРХИИ(&Аренда)
		|	И Контрагенты.Ссылка = &Ссылка";
	
	Запрос.УстановитьПараметр("Аренда", Справочники.Контрагенты.НайтиПоКоду("00-000298"));
	Запрос.УстановитьПараметр("Ссылка", Контрагент);
	
	КонтрагентАренда = Не Запрос.Выполнить().Пустой();
	 ...
	 или СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.БланкиСтрогогоУчета Тогда
		...
		Если НЕ СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку Тогда
			...
			Для каждого СтрокаТаблицы Из ТаблицаПоУслугам Цикл
				...
				Если ПроводкаЕСВ И Не КонтрагентАренда Тогда
					ДвижЕСВ = ПроводкиБУ.Добавить();
					ЗаполнитьЗначенияСвойств(ДвижЕСВ,Проводка);
					Если ДвижЕСВ.СчетДт = ПланыСчетов.Хозрасчетный.АдминистративныеРасходы Тогда
						БухгалтерскийУчет.УстановитьСубконто(ДвижЕСВ.СчетДт, ДвижЕСВ.СубконтоДт, 2, Справочники.СтатьиЗатрат.НайтиПоКоду("100000034"));
					Иначе
						БухгалтерскийУчет.УстановитьСубконто(ДвижЕСВ.СчетДт, ДвижЕСВ.СубконтоДт, 1, Справочники.СтатьиЗатрат.НайтиПоКоду("100000043"));
					КонецЕсли;
					
					ДвижЕСВ.СчетКт       = ПланыСчетов.Хозрасчетный.РасчетыПоПенсионномуОбеспечению;
					// ек++
					//БухгалтерскийУчет.УстановитьСубконто(ДвижЕСВ.СчетКт, ДвижЕСВ.СубконтоКт, 1, Справочники.Налоги.ЕСВГПХ);
					БухгалтерскийУчет.УстановитьСубконто(ДвижЕСВ.СчетКт, ДвижЕСВ.СубконтоКт, 1, Справочники.Налоги.ЕСВФОТГПХ);
					//ек
					
					ДвижЕСВ.Сумма = Проводка.Сумма * 0.22;
					// Test{20240214 
					СтрМинЗП = РегистрыСведений.МинимальнаяОплатаТруда.ПолучитьПоследнее(Дата);
					//ДвижЕСВ.Сумма = Макс(ДвижЕСВ.Сумма, СтрМинЗП.Размер * 0.22);
					ДвижЕСВ.Сумма = Мин(ДвижЕСВ.Сумма, СтрМинЗП.Размер * 15 * 0.22);
					// Test}
				КонецЕсли;	
	...
	Если ПроводкаПДФО И ЗначениеЗаполнено(СчетПДФО) Тогда
		...
		Проводка.Организация  = СтруктураШапкиДокумента.Организация;
		//Проводка.Содержание   = НСтр("ru='Услуги';uk='Послуги'",Локализация.КодЯзыкаИнформационнойБазы());
		Проводка.Сумма        = СуммаДокумента*0.18;
		Проводка.СчетКт       = СчетПДФО;
		//Если КонтрагентАренда Тогда
		//	Проводка.СчетКт       = ПланыСчетов.Хозрасчетный.НайтиПоКоду("6411А");
		//Иначе
		//	Проводка.СчетКт       = ПланыСчетов.Хозрасчетный.НайтиПоКоду("6411 ГПХ");
		//КонецЕсли;
	...
	Если ПроводкаВоенныйСбор И ЗначениеЗаполнено(СчетВоенныйСбор) Тогда
		...
		Проводка.Организация  = СтруктураШапкиДокумента.Организация;
		//Проводка.Содержание   = НСтр("ru='Услуги';uk='Послуги'",Локализация.КодЯзыкаИнформационнойБазы());
		Проводка.Сумма        = СуммаДокумента*0.015;
		
		Проводка.СчетКт       = СчетВоенныйСбор;

		//БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, Справочники.Налоги.НайтиПоКоду("ВоенныйСборДЦПХ"));
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СубконтоВоенныеСбор);
		Проводка.СчетДт       = СтруктураШапкиДокумента.СчетУчетаРасчетовСКонтрагентом;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Контрагенты",СтруктураШапкиДокумента.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Договоры",СтруктураШапкиДокумента.ДоговорКонтрагента);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"ДокументыРасчетовСКонтрагентами",УправлениеВзаиморасчетами.ОпределитьДокументРасчетовСКонтрагентом(Ссылка,Сделка));
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		НовСтр.Номенклатура	= Справочники.Номенклатура.НайтиПоКоду("00000000015");
		
	//...3410_01072020
	// Test{20231026
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.а_ПриказНаПокупку") Тогда
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//ЕРПС.Кононенко
	НаличиеОригинала = Ложь;
	ПолученоВ_ЭлектронномВиде = Ложь;
	Комментарий = "";
	НомерВходящегоДокумента = "";
	ДатаВходящегоДокумента = "";
	СуммаНДСПропорциональноКредит = 0;
	СуммаНДСПропорциональноВсего = 0;
	ЕстьАвансДо01042011 = Ложь;
	СуммаВДВРПоАвансуДо01042011 = 0;
	СвереноПН = Ложь;
	ЕстьТТН = Ложь;
	// Test{20240729 
	//СрокВыполнения = Справочники.а_ПериодыОказанияУслуг.ПустаяСсылка();
	ДокументОснование = Неопределено; 
	Авто = Справочники.Авто.ПустаяСсылка();
	ПунктРазгрузки = Справочники.ПунктыРазгрузки.ПустаяСсылка(); 
	Водитель = Справочники.Сотрудники.ПустаяСсылка();
	Прицеп = Справочники.Прицепы.ПустаяСсылка();
	ЕРПСПеревозчик = Справочники.Контрагенты.ПустаяСсылка();
	ерпсПунктПогрузки = Справочники.Склады.ПустаяСсылка();
	//СчетЕСВ = Неопределено;
	//ПроводкаПДФО = Ложь;
	//СчетПДФО = Неопределено;
	//СубконтоПДФО = Неопределено;
	//ПроводкаВоенныйСбор = Ложь;
	//ПроводкаЕСВ = Ложь;
	//СубконтоВоенныеСбор = Справочники.Налоги.ПустаяСсылка(); 
	// Test}
	//...ЕРПС.Кононенко
