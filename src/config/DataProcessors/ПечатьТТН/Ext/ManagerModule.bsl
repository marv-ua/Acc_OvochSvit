...
Функция ПечатьТТН(ПараметрыПечати, ОбъектыПечати) Экспорт
	...
	"ВЫБРАТЬ
	|	ВложенныйЗапрос.Номенклатура                                 КАК Номенклатура,
	// Администратор{20240518 
	|	ВложенныйЗапрос.ДополнительныйАртикул,		
	// Администратор}
	|	ВложенныйЗапрос.Номенклатура.НаименованиеПолное              КАК ТоварНаименование,
	|	ВложенныйЗапрос.Номенклатура.Код			                 КАК Артикул,
	|	ВложенныйЗапрос.ЕдиницаИзмерения.Представление               КАК БазоваяЕдиницаНаименование,
	...
	|		РеализацияТоваровУслуг.Номенклатура,
	// Администратор{20240518 
	|		РеализацияТоваровУслуг.ДополнительныйАртикул,		
	// Администратор}
	|		РеализацияТоваровУслуг.Коэффициент,
	|		РеализацияТоваровУслуг.ЕдиницаИзмерения,
	|		РеализацияТоваровУслуг.ЕдиницаИзмерения 					 КАК ЕдиницаИзмеренияМест,
...
Процедура ЗаполнитьТТН2021 (Шапка, ЗапросТовары, ТабДокумент, ПараметрыПечати) // ЗаполнитьТТН2021 ерпсНикодаЕвгений 05-11-2021 Процедура заполнения по правилам от 01-10-2021
	...
	//	ОбластьМакета.Параметры.ПлательщикПредставление       = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПокупателе,  "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,МФО,КодПоЕДРПОУ,",, КодЯзыкаПечать);
		...
		Если СведенияОПокупателе.КодПоЕДРПОУ = "39234010" Тогда	//регион продукт
			ОбластьМакета.Параметры.ГрузополучательПредставление  = 
				"Покупець " + "ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЮ ""РЕГІОН-ПРОДУКТ"", Україна, 07301, Київська обл., Вишгородський р-н, м. Вишгород, ""Карат"" промисловий майданчик (пн), буд. 5, корпус А)"+//ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПокупателе,  "ПолноеНаименование,КодПоЕДРПОУ,",, КодЯзыкаПечать) +
				" через " + "ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЮ ""ЮТ КОМПАНІ"" (код ЄДРПОУ 45351684)""" +
				" на в/ч " + Документ.ПунктРазгрузки;
		КонецЕсли;
		Если СведенияОПокупателе.КодПоЕДРПОУ = "45330842" Тогда	//флавкор
			ОбластьМакета.Параметры.ГрузополучательПредставление  = 
			//	"Покупець " +
				"ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЮ ""ЮТ КОМПАНІ"", 49000, Дніпропетровська область, місто  Дніпро, проспект Олександра  Поля, будинок 104А (код ЄДРПОУ 45351684) через ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЮ ""ФЛАВАКОР"", Україна, 07300, Київська обл., Вишгородський р-н, м. Вишгород, пр Мазепи Івана будинок 8 (код ЄДРПОУ 45330842)"+
			//	" через " + "ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЮ ""ЮТ КОМПАНІ"" (код ЄДРПОУ 45351684)""" +
				" на в/ч " + Документ.ПунктРазгрузки;
		КонецЕсли;
	Иначе 
	...
	Для Каждого ВыборкаСтрок Из ЗапросТовары Цикл
		...
		ОбластьМакета.Параметры.Заполнить(ВыборкаСтрок);
		ОбластьМакета.Параметры.НомерСтроки = Ном;
		ОбластьМакета.Параметры.ТоварНаименование = ?(ПараметрыПечати.БезЦен, Строка(ВыборкаСтрок.ДополнительныйАртикул) + ", ","") + ВыборкаСтрок.ТоварНаименование;
