...
//
Асинх Функция вСохранитьЗапросыВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь,ФайлСохранения = Неопределено,ПродолжитьЗапрос = Ложь)
	
	вСохранитьЗапросТекущейСтроки();
	
	АдресВоВременномХранилище = СохранитьДеревоЗапросовВФайлНаСервере();
	
	Если ЗапрашиватьИмяФайла Тогда
		
		СтруктураФайла 		= Ждать РаботаСФайламиКлиент.СохранитьФайлИзВременногоХранилища(АдресВоВременномХранилище, "Файлы запросов (*.sel)|*.sel|Все файлы (*.*)|*.*", ЭтаФорма.ПутьФайлаЗапроса, ЭтаФорма.ИмяФайлаЗапроса);
		
		Если СтруктураФайла = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
		
		ИмяФайлаЗапроса 	= СтруктураФайла.ПолноеИмя;
		ПутьФайлаЗапроса 	= СтруктураФайла.Путь;
		
		//Модифицированность = Ложь;
		//Возврат ЗначениеЗаполнено(СтруктураФайла.ПолноеИмя);
	...
	ИначеЕсли ФайлСохранения <> Неопределено Тогда
		...
		Обещание 			= Ждать ПолучитьФайлССервераАсинх( АдресВоВременномХранилище, ИмяФайлаЗапроса);
	...
	Иначе//Если НЕ ЗначениеЗаполнено(ИмяФайлаЗапроса) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПолучитьФайлЗапроса  = Ждать ПолучитьФайлССервераАсинх(АдресВоВременномХранилище,ИмяФайлаЗапроса);
	Если ЗначениеЗаполнено(ПолучитьФайлЗапроса.ПолноеИмя) Тогда
		Модифицированность = Ложь;
...
Процедура УправлениеДиалогом()
	...
	Если ИмяФайлаЗапроса<>"" Тогда
		Заголовок = "Консоль запитів " + Версия+": "+ИмяФайлаЗапроса;
	Иначе
		Заголовок = "Консоль запитів " + Версия;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Асинх Процедура ЗагрузитьДеревоЗапросовИзФайла(ЗапрашиватьИмяФайла = Ложь)
	
	Если ЗапрашиватьИмяФайла Тогда
		СтруктураФайла 		= Ждать РаботаСФайламиКлиент.ВыбратьФайл(ПутьФайлаЗапроса, ИмяФайлаЗапроса, "Внешние обработки (*.sel)|*.sel|Все файлы (*.*)|*.*");
		
		Если НЕ ЗначениеЗаполнено(СтруктураФайла.ПолноеИмя) Тогда
			Возврат;
		КонецЕсли;
		//
		//Объект.ИмяФайла 	= СтруктураФайла.Имя;
		//Объект.Путь 		= СтруктураФайла.Путь;
		//Объект.ПолноеИмя	= СтруктураФайла.ПолноеИмя;
	...
	//АдресФайлаОбработки	= "";
	//
	//СтруктураФайла 		= Ждать общРаботаСФайламиКлиент.ВыбратьФайл(Объект.Путь, Объект.ПолноеИмя, "Внешние обработки (*.epf)|*.epf|Внешние отчеты (*.erf)|*.erf");
	//
	//Если НЕ ЗначениеЗаполнено(СтруктураФайла.ПолноеИмя) Тогда
	//	Возврат;
	//КонецЕсли;
	...
	//Объект.ПолноеИмя	= СтруктураФайла.ПолноеИмя;
	//
	//ОбработатьЗагрузкуФайлаОбработки(Объект.ПолноеИмя);
	...
	Описание 				= Ждать ПоместитьФайлНаСерверАсинх(,,, ИмяФайлаЗапроса, Новый УникальныйИдентификатор);
	АдресВХранилище = Описание.Адрес;
