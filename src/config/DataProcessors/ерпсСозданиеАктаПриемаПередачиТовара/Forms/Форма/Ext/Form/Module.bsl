...
Процедура ОбработчикОкончанияПомещения(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		АдресЗагруженныхДанных = ПолучитьТаблицуФайла(Адрес, УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ПолучитьТаблицуФайла(Адрес, УникальныйИдентификатор)
	...
	ПО_данные.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТабДок.Область(1, 1, ТабДок.ВысотаТаблицы, ТабДок.ШиринаТаблицы));
	ПО_данные.Выполнить();
...
//	Т.Колонки.Добавить("ПунктРазгрузки", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ПунктыРазгрузки"))));
	...
	Т.Колонки[0].Имя = "ИДЗаявкиКлиента";
	Т.Колонки[2].Имя = "ДатаОтгрузки";
	Т.Колонки[3].Имя = "ПунктРазгрузкиПоиск";
	Т.Колонки[13].Имя = "ЗагруженныйДополнительныеАртикулы";
	Т.Колонки[14].Имя = "ЗагруженнаяНоменклатура";
	Т.Колонки[15].Имя = "ЗагруженноеКоличество";
	...
	Для Каждого Стр Из Т Цикл
		Стр.ЗагруженныйДополнительныеАртикулы = СтрЗаменить(СтрЗаменить(СокрЛП(Стр.ЗагруженныйДополнительныеАртикулы), " ", ""), Символы.НПП, "");
		Стр.ИДЗаявкиКлиента = СокрЛП(Стр.ИДЗаявкиКлиента);
	КонецЦикла;
	
	Возврат ПоместитьВоВременноеХранилище(Т, УникальныйИдентификатор);	
КонецФункции
// Test}