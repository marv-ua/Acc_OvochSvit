...
Процедура ЗаполнитьВыбранныеМеста(МестаДоставки)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |	ПунктыРазгрузки.НаименованиеПолное КАК НаименованиеПолное
	                      |ИЗ
...
Процедура ПослеЗаписиНовогоЗавершение(Параметр1, Параметр2) Экспорт
	...
	Массив = Новый Массив;
	Для Каждого Эл Из ВыбранныеМеста Цикл
		Если Эл.Выбрана Тогда
			Массив.Добавить(Эл.Ссылка);
		КонецЕсли;
	КонецЦикла;
	
	ВыбранныеМеста.Очистить();
	ЗаполнитьВыбранныеМеста(Массив);//Новый Массив);
