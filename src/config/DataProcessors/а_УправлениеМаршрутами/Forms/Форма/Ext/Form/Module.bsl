...
Процедура РезультатПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ЗначениеЗаполнено(Результат.Область(Область.Имя).Значение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("Не можна змінювати дату двічі. Попередньо потрібно переформувати звіт"));
		Возврат;
	КонецЕсли;
